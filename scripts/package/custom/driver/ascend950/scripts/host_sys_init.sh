...
wait_ub_device_init() {
    # 该功能跟随后续通信维测需求开发，当前以打桩形式预埋
    source "${targetdir}"/driver/script/specific_func.inc
    get_ub_info
    for i in $(seq 1 400)
    do
        local dev_num=$(ls /dev/davinci* 2>/dev/null | grep -cE "/dev/davinci[0-9]{1,2}$")
        # 打桩，不管单p多p，等1台设备上线后就OK
...
init_signature_configuration() {
    ...
    for devid in "${devids[@]}"; do
       if [ ! -f "/var/asdrv/dev${devid}/device-sw-plugin/verify/flag" ];then
           ...
           chmod 750 /var/asdrv/dev${devid}

           mkdir -p /var/asdrv/dev${devid}/device-sw-plugin
           chmod 750 /var/asdrv/dev${devid}/device-sw-plugin

           mkdir -p /var/asdrv/dev${devid}/device-sw-plugin/verify
           chmod 750 /var/asdrv/dev${devid}/device-sw-plugin/verify
		   ...
		   chmod 640 "/var/asdrv/dev${devid}/device-sw-plugin/verify/flag";
           chmod 640 "/var/asdrv/dev${devid}/device-sw-plugin/verify/user-certificate";
        fi
