...
屏蔽指定AI Core上的任务执行，以便排查哪个AI Core故障。建议单算子调用场景下使用该命令排查问题。
...
|--aic_switch <switchflag>|必选|是否屏蔽AI Core，取值说明如下：<br>- 0：屏蔽AI Core。设置为该值后，若重启运行环境，默认不屏蔽AI Core。<br>- 1：不屏蔽AI Core，默认该选项。|
|--coreid <coreid>|必选|指定core id，多个core id之间以英文逗号分隔，最多可同时指定4个AI Core，只要有一个core id无效，则该命令会返回报错。<br>若想恢复初始值，可将--aic_switch设置为1，将--coreid设置为0xFFFF。|
