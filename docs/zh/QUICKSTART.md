...
3.&emsp;***[开发指南](#开发指南)***：自定义驱动开发指南，学习从零开发、编译及验证代码。
...
a. 重复执行该编译命令之前，需要手动清理上一次编译产生的文件，否则可能会有缓存。
...
b. ascend950支持在灵衢计算系统超节点架构的ARM环境编译；进行源码编译前，需要额外完成以下环境准备工作：

- 编译环境要求为`openEuler 24.03 LTS SP4`系统
- 安装ube相关依赖包
