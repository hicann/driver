...
STATIC int32_t sched_sysfs_clear_node_stat(struct sched_numa_node *node)
{
    ...
    ret = memset_s(&node->event_trace, sizeof(struct sched_node_event_trace), 0, sizeof(struct sched_node_event_trace));
    if (ret != 0) {
    ...
    for (i = 0; i < SCHED_MAX_CHIP_NUM; i++) {
        ...
        if (node != NULL) {
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%u,%u,%u\n", i,
                             node->cpu_num, node->sched_cpu_num);
            if (ret >= 0) {
                offset += ret;
            }
            esched_dev_put(node);
    ...
    for (i = 0; i < SCHED_MAX_CHIP_NUM; i++) {
        ...
        if (node != NULL) {
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%u,%u,%u,%u,%u\n",
                             i, node->curr_msg.msg_type, node->curr_msg.dst_pid, node->curr_msg.dst_event_id,
                             node->curr_msg.dst_subevent_id);
            if (ret >= 0) {
                offset += ret;
            }
            esched_dev_put(node);
...
STATIC ssize_t sched_sysfs_node_id_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf, size_t count)
{
    u32 val = 0;
    struct sched_numa_node *node = NULL;
...
STATIC ssize_t sched_sysfs_node_debug_read(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(
        ...
        "bit0: clear node event trace, bit1:"
...
STATIC ssize_t sched_sysfs_node_debug_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf,
                                            size_t count)
{
    u32 val = 0;
    int32_t ret;
...
STATIC ssize_t sched_sysfs_node_cpu_list(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    for (i = 0; i < node->sched_cpu_num; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                         "%u,(%d,%u,%u,%u,%u,%u,%llu,%llu)\n", cpu_ctx->cpuid, record->thread_info.pid,
                         record->thread_info.gid, record->thread_info.task_id, record->thread_info.tid,
...
STATIC void sched_sysfs_node_get_proc_list(char *buf, ssize_t *offset, struct sched_proc_ctx *proc_ctx)
{
    int ret = snprintf_s(buf + *offset, KA_MM_PAGE_SIZE - *offset, KA_MM_PAGE_SIZE - *offset - 1, "%d,%s,%d,%d,%d\n",
                         ...
                         ka_base_atomic_read(&proc_ctx->publish_event_num),
                         ka_base_atomic_read(&proc_ctx->sched_event_num));
    if (ret >= 0) {
        *offset += ret;
    }
}
...
STATIC ssize_t sched_sysfs_event_que_trace(struct sched_event *event_base, struct sched_event_que *que, char *buf,
                                           ssize_t offset, const char *que_name, u32 que_id)
{
    u32 i, j, flag;
    int32_t ret;
    ...
    for (i = 0; i < que->depth; i++) {
        ...
        if (flag == SCHED_INVALID) {
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                             "%s(%u),%u,%u,%u,%u,%u\n", que_name, que_id, i, event->trace.type, event->trace.a,
                             event->trace.b, event->trace.c);
            if (ret >= 0) {
                offset += ret;
            }
        }
...
STATIC ssize_t sched_sysfs_sample_period_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf,
                                               size_t count)
{
    u32 val = 0;
    struct sched_numa_node *node = NULL;
                                               ...
                                               struct sched_event_sample_data *data)
{
    ...
    if (ka_base_strlen(period) > 0) {
        ret = snprintf_s(buf + offset, buf_len - offset, buf_len - offset - 1, "%s,%llu,%d,%d", period,
                         data[idx].timestamp, submit_speed, sched_speed);
    } else {
        ret = snprintf_s(buf + offset, buf_len - offset, buf_len - offset - 1, "%llu,%llu,%llu", data[idx].timestamp,
                         total_event_num, data[idx].cur_event_num);
    }
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < SCHED_MAX_EVENT_TYPE_NUM; i++) {
        if (ka_base_strlen(period) > 0) {
            ...
            ret = snprintf_s(buf + offset, buf_len - offset, buf_len - offset - 1, ",(%d,%llu,%llu)", i,
                             data[idx].publish_event_num[i] - data[pre_index].publish_event_num[i],
                             data[idx].publish_event_num[i] - data[idx].sched_event_num[i]);
        }
        if (ret >= 0) {
            offset += ret;
        }
                                                    ...
                                                    struct sched_event_sample *sample_data, SAMPLE_TYPE_VALUE type)
{
    ...
    ret = snprintf_s(buf, KA_MM_PAGE_SIZE, KA_MM_PAGE_SIZE - 1,
                     "timestamp, publish_event_num, cur_event_num,"
...
STATIC ssize_t sched_sysfs_cpu_usage_sample_format(struct sched_cpu_sample *sample_data, char *buf, u32 buf_len,
                                                   u32 idx, u32 pre_index)
{
    int32_t usage;
...
STATIC ssize_t sched_sysfs_node_sched_abnormal_time_write(const char *buf, size_t count, u64 *timestamp_thres)
{
    u32 val = 0;
...
STATIC ssize_t sched_sysfs_node_sched_proc_abnormal_time_read(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    size_t count = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
    if (node != NULL) {
...
STATIC ssize_t sched_sysfs_node_sched_proc_abnormal_time_write(ka_device_t *dev, ka_device_attribute_t *attr,
...
STATIC ssize_t sched_sysfs_node_sched_publish_syscall_abnormal_time_read(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                         char *buf)
{
    size_t count = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
...
STATIC ssize_t sched_sysfs_node_sched_publish_syscall_abnormal_time_write(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                          const char *buf, size_t count)
{
    struct sched_numa_node *node = esched_dev_get(g_node_id);
    if (node != NULL) {
...
STATIC ssize_t sched_sysfs_node_sched_wakeup_abnormal_time_read(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                char *buf)
{
    size_t count = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
...
STATIC ssize_t sched_sysfs_node_sched_wakeup_abnormal_time_write(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                 const char *buf, size_t count)
{
    struct sched_numa_node *node = esched_dev_get(g_node_id);
    if (node != NULL) {
...
STATIC ssize_t sched_sysfs_node_sched_thread_run_abnormal_time_read(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                    char *buf)
{
    size_t count = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
...
STATIC ssize_t sched_sysfs_node_sched_thread_run_abnormal_time_write(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                     const char *buf, size_t count)
{
    struct sched_numa_node *node = NULL;
    u32 val = 0;
...
STATIC int32_t sched_sysfs_node_single_event_info(char *buf, struct sched_abnormal_event_item *event_info,
                                                  struct sched_event_timestamp *timestamp, ssize_t *offset, u32 idx,
                                                  ABNORMAL_EVENT_TYPE_VALUE type)
{
    u64 wakeup_to_waked;
    u64 wakeup_offset;
    s64 subscribe_in_kernel;
    int32_t ret;
    char *format_string[] = {"%d,%llu,%llu,%llu,%llu,%llu,%lld,%llu,%llu,%llu,(%llu),%d,%d,%d,%d,%d,%d,%d,%d,%d,%s\n",
    ...
    ret = snprintf_s(buf + *offset, KA_MM_PAGE_SIZE - *offset, KA_MM_PAGE_SIZE - *offset - 1, format_string[type], idx,
                     timestamp->publish_user, timestamp->publish_user_of_day,
                     ...
                     tick_to_microsecond(event_info->proc_time), event_info->event_id, event_info->publish_pid,
                     event_info->publish_cpuid, event_info->wait_event_num_in_que, event_info->pid, event_info->gid,
                     event_info->tid, event_info->bind_cpuid, event_info->kernel_tid, event_info->name);
    if (ret >= 0) {
        *offset += ret;
    }
...
STATIC ssize_t sched_sysfs_node_event_all_info(char *buf, struct sched_abnormal_event_item *events, int32_t cur_id,
                                               u32 event_num, ABNORMAL_EVENT_TYPE_VALUE type)
{
...
STATIC ssize_t sched_sysfs_node_sched_abnormal_event_clear(ka_device_t *dev, ka_device_attribute_t *attr,
                                                           const char *buf, size_t count)
{
    u32 val = 0;
    ...
    if (val == 1) {
        ...
        abnormal_event->publish_in_kernel.timestamp_thres = microsecond_to_tick(
            SCHED_DEFAULT_PUBLISH_IN_KERNEL_TIME_THRES);
        abnormal_event->wakeup.timestamp_thres = microsecond_to_tick(SCHED_DEFAULT_WAKEUP_TIME_THRES);
        abnormal_event->publish_subscribe.timestamp_thres = microsecond_to_tick(
            SCHED_DEFAULT_PUBLISH_SUBSCRIBE_TIME_THRES);
...
STATIC ssize_t sched_sysfs_node_sched_proc_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    if (node != NULL) {
        offset = sched_sysfs_node_event_all_info(buf, node->abnormal_event.proc.event_info,
                                                 ka_base_atomic_read(&node->abnormal_event.proc.cur_index),
...
STATIC ssize_t sched_sysfs_node_sched_thread_run_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                char *buf)
{
    ssize_t offset = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
...
STATIC ssize_t sched_sysfs_node_sched_publish_syscall_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                     char *buf)
{
    static u32 i = 0; /* output msg is large than 1 page */
    int32_t ret;
    ...
    for (; i < SCHED_ABNORMAL_EVENT_MAX_NUM; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                         "%d,%llu,%llu,%d,%d,%d\n", i, timestamp->publish_user,
                         tick_to_microsecond(timestamp->publish_in_kernel - timestamp->publish_user),
...
STATIC ssize_t sched_sysfs_node_sched_publish_in_kernel_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                       char *buf)
{
    static u32 i = 0; /* output msg is large than 1 page */
    int32_t ret;
...
STATIC ssize_t sched_sysfs_node_sched_wakeup_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ssize_t offset = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
    if (node != NULL) {
        offset = sched_sysfs_node_event_all_info(buf, node->abnormal_event.wakeup.event_info,
                                                 ka_base_atomic_read(&node->abnormal_event.wakeup.cur_index),
                                                 SCHED_ABNORMAL_EVENT_MAX_NUM, WAKEUP_ABNORMAL_EVENT);
        esched_dev_put(node);
    }
...
STATIC ssize_t sched_sysfs_node_sched_publish_subscribe_abnormal_event(ka_device_t *dev, ka_device_attribute_t *attr,
                                                                       char *buf)
{
    ssize_t offset = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
    if (node != NULL) {
        offset = sched_sysfs_node_event_all_info(buf, node->abnormal_event.publish_subscribe.event_info,
                                                 ka_base_atomic_read(&node->abnormal_event.publish_subscribe.cur_index),
                                                 SCHED_ABNORMAL_EVENT_MAX_NUM, PUBLISH_SUBSCRIBE_ABNORMAL_EVENT);
        esched_dev_put(node);
    }
...
STATIC ssize_t sched_sysfs_node_sched_event_list(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    if (node != NULL) {
        offset = sched_sysfs_node_event_all_info(buf, node->event_trace.event_info,
                                                 ka_base_atomic_read(&node->event_trace.cur_index),
                                                 SCHED_EVENT_TRACE_MAX_NUM, NORMAL_EVENT);
        esched_dev_put(node);
    }
...
STATIC ssize_t sched_sysfs_node_event_summary(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(
        ...
        perf_stat->total_publish_fail_event_num, perf_stat->total_sched_event_num, perf_stat->total_wakeup_event_num,
        wakeup_err_times);
    if (ret >= 0) {
        offset += ret;
    }
    ...
    ret = snprintf_s(
        buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
        "\nevent "
        "id,submit_event_num,submit_fail_event_num,sw_publish_event_num,hw_publish_event_num,publish_fail_event_num,"
        "sched_event_num\n");
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < SCHED_MAX_EVENT_TYPE_NUM; i++) {
        if ((perf_stat->sw_publish_event_num[i] != 0) || (perf_stat->sched_event_num[i] != 0) ||
            ...
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                             "%d, %llu, %llu, %llu, %llu, %llu, %llu\n", i, perf_stat->submit_event_num[i],
                             perf_stat->submit_fail_event_num[i], perf_stat->sw_publish_event_num[i],
                             perf_stat->hw_publish_event_num[i], perf_stat->publish_fail_event_num[i],
                             perf_stat->sched_event_num[i]);
            if (ret >= 0) {
                offset += ret;
            }
        }
    ...
    if (wakeup_err_times != 0) {
        ...
        for (i = 0; i < dump_times; i++) {
            ...
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                             "%u, %u, %u, %u, %u, %u, %u, %u\n", err_info->thread_status, err_info->bind_cpuid,
                             err_info->pid, err_info->group_id, err_info->tid, err_info->kernel_tid,
                             err_info->normal_wakeup_reason, err_info->pre_normal_wakeup_reason);
            if (ret >= 0) {
                offset += ret;
            }
        }
...
STATIC ssize_t sched_sysfs_node_cpuid_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf,
                                            size_t count)
{
    u32 val = 0;
    struct sched_numa_node *node = esched_dev_get(g_node_id);
...
STATIC ssize_t sched_sysfs_node_cur_event_num(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    for (i = 0; i < SCHED_MAX_PROC_PRI_NUM; i++) {
        for (j = 0; j < SCHED_MAX_EVENT_PRI_NUM; j++) {
            ...
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%u,%d,%d,%u",
                             node->node_id, i, j, event_list->cur_num);
            if (ret >= 0) {
                offset += ret;
            }
...
STATIC ssize_t sched_sysfs_node_cpu_cur_thread(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    for (i = 0; i < node->sched_cpu_num; i++) {
        ...
        if (thread_ctx != NULL) {
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                             "%u,pid:%d(%s),gid:%u,tid:%u\n", i, thread_ctx->grp_ctx->pid,
                             thread_ctx->grp_ctx->proc_ctx->name, thread_ctx->grp_ctx->gid, thread_ctx->tid);
            if (ret >= 0) {
                offset += ret;
            }
...
STATIC ssize_t sched_sysfs_node_cpu_event_resource(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    for (i = 0; i < node->cpu_num; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                         "%u,%u:%u:%u:%u:%u:%llu:%llu\n", i, res_que->depth,
                         res_que->depth - sched_que_element_num(res_que), res_que->head, res_que->tail,
                         res_que->stat.max_use, res_que->stat.enque_full, res_que->stat.deque_empty);
        if (ret >= 0) {
            offset += ret;
        }
                                                        ...
                                                        struct sched_grp_ctx *grp_ctx, ssize_t *offset, int32_t idx)
{
    ...
    if (grp_ctx->sched_mode == SCHED_MODE_SCHED_CPU) {
        ret = snprintf_s(buf + *offset, KA_MM_PAGE_SIZE - *offset, KA_MM_PAGE_SIZE - *offset - 1,
                         ...
                         thread_ctx->bind_cpuid, grp_ctx->pid, grp_ctx->gid, idx, thread_ctx->name,
                         thread_ctx->kernel_tid, status, thread_ctx->wait_flag, thread_ctx->timeout_flag,
                         ...
                         (thread_ctx->stat.sched_event != 0) ?
                             (tick_to_microsecond(thread_ctx->total_proc_time) / thread_ctx->stat.sched_event) :
                             0,
                         tick_to_microsecond(thread_ctx->max_sched_time),
                         (thread_ctx->stat.sched_event != 0) ?
                             (tick_to_microsecond(thread_ctx->total_sched_time) / thread_ctx->stat.sched_event) :
                             0,
                         thread_ctx->subscribe_event_bitmap, thread_ctx->stat.sched_event, thread_ctx->stat.timeout_cnt,
                         ka_base_atomic_read(&thread_ctx->stat.discard_event));
    } else {
        ret = snprintf_s(buf + *offset, KA_MM_PAGE_SIZE - *offset, KA_MM_PAGE_SIZE - *offset - 1,
                         "%d,%s,%u,%s,%u,%llu,%llu,%llu,%llu,%llu,%llx,%u,%u\n", idx, thread_ctx->name,
                         thread_ctx->kernel_tid, status, thread_ctx->wait_flag,
                         tick_to_millisecond(thread_ctx->start_time), tick_to_microsecond(thread_ctx->max_proc_time),
                         (thread_ctx->stat.sched_event != 0) ?
                             (tick_to_microsecond(thread_ctx->total_proc_time) / thread_ctx->stat.sched_event) :
                             0,
                         ...
                         (thread_ctx->stat.sched_event != 0) ?
                             (tick_to_microsecond(thread_ctx->total_sched_time) / thread_ctx->stat.sched_event) :
                             0,
                         thread_ctx->subscribe_event_bitmap, ka_base_atomic_read(&grp_ctx->cur_event_num),
...
STATIC ssize_t sched_sysfs_node_cpu_sched_track_format(struct sched_cpu_sched_trace *sched_trace, char *buf,
                                                       u32 buf_len, u32 start_index, u32 num, u64 *last_timestamp)
{
    u32 i, idx;
    int32_t ret;
    ...
    for (i = 0; i < num; i++) {
        ...
        ret = snprintf_s(buf + offset, buf_len - offset, buf_len - offset - 1,
                         ...
                         sched_thread->sched_timestamp, sched_thread->publish_timestamp,
                         sched_thread->add_queue_list_timestamp, sched_thread->pid, sched_thread->gid,
                         sched_thread->tid, sched_thread->event_id, sched_thread->pid_pri, sched_thread->event_pri,
                         sched_thread->normal_wakeup_reason, use_time, sched_thread->cur_event_num,
                         tick_to_microsecond(sched_thread->wait_time),
...
STATIC void sched_cpu_trace_record(struct sched_cpu_ctx *cpu_ctx, u64 trace_num,
                                   const struct sched_trace_record_info *trace_record)
{
    ...
    ret = snprintf_s(buf, MAX_LENTH, MAX_LENTH - 1, "%s/cpu_%d_%s_%s_%llu", SCHED_FILE_PATH, cpu_ctx->cpuid,
                     trace_record->record_reason, trace_record->key, trace_record->timestamp);
    if (ret < 0) {
        sched_err("Failed to invoke the snprintf_s.\n");
        return;
    }
    ...
    ret = snprintf_s(
        sched_sysfs_buf, KA_MM_PAGE_SIZE, KA_MM_PAGE_SIZE - 1,
        "cpuid %d thread schedule track\n"
...
STATIC ssize_t sched_sysfs_proc_group_id_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf,
                                               size_t count)
{
    u32 val = 0;
...
STATIC ssize_t sched_sysfs_node_proc_group_event_list(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    for (i = 0; i < SCHED_MAX_EVENT_TYPE_NUM; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%u,%u(%u", i,
                         event_thread_map->thread_num, event_thread_map->thread[0]);
        if (ret >= 0) {
            offset += ret;
        }
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "),%u,%u,%u\n",
                         grp_ctx->max_event_num[i], ka_base_atomic_read(&grp_ctx->event_num[i]),
                         ka_base_atomic_read(&grp_ctx->drop_event_num[i]));
        if (ret >= 0) {
            offset += ret;
        }
    }
...
STATIC ssize_t sched_sysfs_node_proc_group_thread_inner(struct sched_grp_ctx *grp_ctx, char *buf, int32_t mode)
{
    ...
    if (grp_ctx->sched_mode == SCHED_MODE_SCHED_CPU) {
        ret = snprintf_s(
            buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
            "bind_cpuid,pid,gid,tid,name,kernel_tid,status,wait flag,timeout flag,pre_wakeup reason,normal wakeup "
            "reason,"
            "start time(ms),max proc time(us),ave proc time(us),max sched time(us),ave sched time(us),"
            "subscribe event bitmap,fwd event,sched event,timeout cnt,discard cnt\n");
    } else {
...
STATIC ssize_t sched_sysfs_group_cur_event_num(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                     ...
                     cur_pid, cur_gid, ka_base_atomic_read(&grp_ctx->cur_event_num));
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < SCHED_MAX_EVENT_PRI_NUM; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%d,%u,%d,%u\n", cur_pid,
                         cur_gid, i, event_list->cur_num);
        if (ret >= 0) {
            offset += ret;
        }
...
ssize_t sched_sysfs_node_vf_list(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                     "vf_id, status, resource_packet_num.\n");
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < VMNG_VDEV_MAX_PER_PDEV; i++) {
        ...
        ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1, "%5d, %s, %llx.\n", i,
                         msg, vf_ctx->config_sched_cpu_mask);
        if (ret >= 0) {
            offset += ret;
        }
    }
...
ssize_t sched_sysfs_vf_info_show(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                     ...
                     ka_base_atomic_read(&vf_ctx->proc_num), vf_ctx->que_depth, vf_ctx->config_sched_cpu_mask,
                     vf_ctx->sched_cpu_mask);
    if (ret >= 0) {
        offset += ret;
    }
    ...
    ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                     ...
                     g_cur_vf, ka_base_atomic64_read(&vf_ctx->stat.publish_event_num),
                     ka_base_atomic64_read(&vf_ctx->stat.sched_event_num),
...
ssize_t sched_sysfs_vf_occupy_cpu(ka_device_t *dev, ka_device_attribute_t *attr, char *buf)
{
    ...
    ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                     "vf %u(status %u) max occupy aicpu\n"
                     "config sched cpu mask: 0x%llx sched cpu mask: 0x%llx\n",
                     g_cur_vf, ka_base_atomic_read(&vf_ctx->status), config_sched_cpu_mask, vf_ctx->sched_cpu_mask);
    if (ret >= 0) {
        offset += ret;
    }
    ...
    for (i = 0; i < node->sched_cpu_num; i++) {
        ...
        if (config_sched_cpu_mask & (0x1ULL << node->sched_cpuid[i])) {
            ret = snprintf_s(buf + offset, KA_MM_PAGE_SIZE - offset, KA_MM_PAGE_SIZE - offset - 1,
                             "aicpu:%u, pid:%d(%s), gid:%u, tid:%u\n", cpu_ctx->cpuid, thread_ctx->grp_ctx->pid,
                             thread_ctx->grp_ctx->proc_ctx->name, thread_ctx->grp_ctx->gid, thread_ctx->tid);
            if (ret >= 0) {
                offset += ret;
            }
...
STATIC ssize_t sched_sysfs_record_num_write(ka_device_t *dev, ka_device_attribute_t *attr, const char *buf,
                                            size_t count)
{
    u32 val = 0;
...
#define DECLEAR_ESCHED_FS_ATTR                                                                                         \
    ...
    static DRV_FS_ATTR(node_sched_proc_abnormal_time_in_microsecond, SCHED_ATTR_RW,                                    \
                       sched_sysfs_node_sched_proc_abnormal_time_read,                                                 \
                       sched_sysfs_node_sched_proc_abnormal_time_write);                                               \
    static DRV_FS_ATTR(node_sched_abnormal_event_clear, SCHED_ATTR_WR, NULL,                                           \
                       sched_sysfs_node_sched_abnormal_event_clear);                                                   \
    static DRV_FS_ATTR(node_sched_proc_abnormal_event, SCHED_ATTR_RD, sched_sysfs_node_sched_proc_abnormal_event,      \
                       NULL);                                                                                          \
    ...
    static DRV_FS_ATTR(node_sched_wakeup_abnormal_time_in_microsecond, SCHED_ATTR_RW,                                  \
                       sched_sysfs_node_sched_wakeup_abnormal_time_read,                                               \
                       sched_sysfs_node_sched_wakeup_abnormal_time_write);                                             \
    static DRV_FS_ATTR(node_sched_wakeup_abnormal_event, SCHED_ATTR_RD, sched_sysfs_node_sched_wakeup_abnormal_event,  \
                       NULL);                                                                                          \
...
#define DECLEAR_ESCHED_FS_ATTR_NORMAL_LIST(_struct_type, _profix)                            \
    static _struct_type *g_sched_##_profix##_normal_list[] = {                               \
...
#define DECLEAR_ESCHED_FS_VFIO_ATTR_LIST(_struct_type, _profix)                          \
    static _struct_type *g_sched_##_profix##_vfio_attrs[] = {                            \
        DRV_FS_ATTR_NAME_POINTER(vfid),          DRV_FS_ATTR_NAME_POINTER(vf_proc_list), \
        DRV_FS_ATTR_NAME_POINTER(vf_list),       DRV_FS_ATTR_NAME_POINTER(vf_info_show), \
        DRV_FS_ATTR_NAME_POINTER(vf_occupy_cpu), NULL,                                   \
    }
#endif

DECLEAR_ESCHED_FS_ATTR;

DECLEAR_ESCHED_FS_ATTR_NORMAL_LIST(ka_attribute_t, sysfs);

static const ka_attribute_group_t g_sched_sysfs_node_normal_group = {.attrs = g_sched_sysfs_normal_list,
                                                                     .name = "node"};

#ifdef CFG_FEATURE_VFIO
DECLEAR_ESCHED_FS_VFIO_ATTR;

DECLEAR_ESCHED_FS_VFIO_ATTR_LIST(ka_attribute_t, sysfs);

static const ka_attribute_group_t g_sched_sysfs_node_vfio_group = {.attrs = g_sched_sysfs_vfio_attrs, .name = "vfio"};
#endif
