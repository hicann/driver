...
static int dms_event_converge_occur_add_to_cb(struct dms_dev_ctrl_block *dev_cb, DMS_EVENT_NODE_STRU *exception_node);
...
static int dms_event_obj_to_event_name(struct dms_event_obj *event_obj, char *event_name, u32 name_len)
{
    ...
    ret = dms_get_sensor_type_name(event_obj->event.sensor_event.sensor_type, sensor_type, DMS_EVENT_NAME_BUFF_LEN);
    if (ret != 0) {
        dms_err("Get name of sensor_type failed. (devid=%u; sensor_type=%u; ret=%d)\n", event_obj->deviceid,
                event_obj->event.sensor_event.sensor_type, ret);
        return ret;
    }

    ret = dms_get_event_string(event_obj->event.sensor_event.sensor_type, event_obj->event.sensor_event.event_state,
    ...
    ret = snprintf_s(event_name, DMS_MAX_EVENT_NAME_LENGTH, DMS_MAX_EVENT_NAME_LENGTH - 1,
                     ...
                     "time=%llu ms, event assertion=%u.",
                     node_type, event_obj->node_id, event_obj->sub_node_id, sensor_type,
...
static int dms_event_obj_to_event_code(struct dms_event_obj *event_obj, DMS_EVENT_NODE_STRU *exception_node)
{
    unsigned int owner_node_type;
    ...
    exception_node->event.event_code |= DMS_EVENT_OBJ_NODETYPE_TO_CODE(owner_node_type);
    exception_node->event.event_code |= DMS_EVENT_OBJ_SENSOR_TYPE_TO_CODE(event_obj->event.sensor_event.sensor_type);
    exception_node->event.event_code |= DMS_EVENT_OBJ_EVENT_STATE_TO_CODE(event_obj->event.sensor_event.event_state);
    return DRV_ERROR_NONE;
}

static int dms_event_obj_to_event_node(struct dms_event_obj *event_obj, DMS_EVENT_NODE_STRU *exception_node)
{
    int ret;
    ...
    if (ret != 0) {
        dms_err("Get name of sensor_num failed. (devid=%u; sensor_num=%u; ret=%d)\n", event_obj->deviceid,
                event_obj->event.sensor_event.sensor_num, ret);
        return ret;
    }
    return DRV_ERROR_NONE;
}

int dms_event_obj_to_exception(struct dms_event_obj *event_obj, DMS_EVENT_NODE_STRU *exception_node)
{
    int ret;
                                     ...
                                     DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    if (reported_list->reported_num > DMS_MAX_EVENT_NUM) {
        dms_err("reported_num is too many. (reported_num=%u; max=%u)\n", reported_list->reported_num,
                DMS_MAX_EVENT_NUM);
        ka_task_mutex_unlock(&reported_list->lock);
        return DRV_ERROR_EVENT_NOT_MATCH;
    }
...
static inline void dms_event_get_attribute_info(u32 devid, char opt, EVENT_CONVERGE_NODE_T *convergent_node,
                                                int *attribute_info)
{
...
static int dms_event_print_convergent_next_layer(EVENT_CONVERGE_NODE_T *parent_node, u32 devid, char opt, int *avl_len,
                                                 char **str)
{
        ...
        if (current_node != NULL) {
            ...
            *avl_len -= len;
            ...
            while ((protect_j++ <= EVENT_CONVERGE_ID_NUM_MAX) && (current_bro != current_node)) {
                ...
                len = snprintf_s(*str, *avl_len, *avl_len - 1, " - 0x%X(%c:%d)", current_bro->event_id, opt,
                                 attribute_info);
                EVENT_DFX_CHECK_DO_SOMETHING(len < 0, return len);
                *str += len;
                *avl_len -= len;
...
STATIC int dms_event_print_converge_one_tree(EVENT_CONVERGE_NODE_T *top_node, u32 devid, char opt, int *avl_len,
                                             char **str)
{
...
ssize_t dms_event_print_convergent_diagrams(u32 devid, char opt, char *str)
{
    ...
    len = snprintf_s(str, avl_len, avl_len - 1, "Convergent diagrams config: (switch=%s; validity=%s; dev_id=%u)\n",
                     dms_event_is_converge() ? "on" : "off", g_converge_htable.converge_validity ? "true" : "false",
                     devid);
    ...
    if ((devid >= ASCEND_DEV_MAX_NUM) || !EVENT_CONVERGE_OPT_IS_VALID(opt)) {
        len = snprintf_s(str, avl_len, avl_len - 1,
                         "Invalid parameter, opt:a(assertion), m(mask), "
...
static int dms_event_convergent_item_check(unsigned int item_converge_id[], unsigned int event_id_num)
{
    ...
    if ((event_id_num < EVENT_CONVERGE_ID_NUM_MIN) || (event_id_num > EVENT_CONVERGE_ID_NUM_MAX)) {
        dms_err("Invalid event_id_num. (event_id_num=%u; max=%u)\n", event_id_num, EVENT_CONVERGE_ID_NUM_MAX);
...
static unsigned int dms_event_convergent_item_fix(unsigned int item_converge_id[], unsigned int event_id_num)
{
    EVENT_CONVERGE_NODE_T *event_node = NULL;
...
STATIC bool dms_event_convergent_diagrams_layer_check(void)
{
    ...
    ka_hash_for_each_safe(g_converge_htable.htable, bkt, local_node, convergent_node, hnode)
    {
        ...
        if (layer > EVENT_CONVERGE_DIAGRAMS_LAYER_MAX) {
            ka_task_mutex_unlock(&g_converge_htable.lock);
            dms_warn("Convergent diagrams layer is invalid. (layer=%u; max=%u)\n", layer,
                     EVENT_CONVERGE_DIAGRAMS_LAYER_MAX);
            return false;
        }
    }
    ka_task_mutex_unlock(&g_converge_htable.lock);
    ...
    for (i = 0; i < ASCEND_DEV_MAX_NUM; i++) {
        ...
        ka_list_for_each_safe(pos, n, &converge_list->head)
        {
            ...
            converge_list->health_code = event_node->event.severity > converge_list->health_code ?
                                             event_node->event.severity :
                                             converge_list->health_code;
        }
...
void dms_event_mask_add_to_event_cb(u32 phyid, struct dms_dev_ctrl_block *dev_cb, u32 event_id)
{
    ...
    if (memcpy_s(&exception_data, sizeof(DMS_EVENT_NODE_STRU), convergent_node->exception_data[phyid],
                 sizeof(DMS_EVENT_NODE_STRU)) != 0) {
        ka_task_mutex_unlock(&g_converge_htable.lock);
        return;
    }
    ka_task_mutex_unlock(&g_converge_htable.lock);
    ...
    if (dms_event_distribute_handle(&exception_data, DMS_DISTRIBUTE_PRIORITY0)) {
        dms_err("Distribute exception failed. (dev_id=%u; event_id=0x%x)\n", exception_data.event.deviceid,
                exception_data.event.event_id);
    }
}
...
STATIC int dms_event_obj_converge_occur(DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    do {
        ...
        if ((convergent_node->parent == NULL) && (convergent_node->exception_data[deviceid] == NULL)) {
            ...
            (void)memcpy_s(convergent_node->exception_data[deviceid], sizeof(DMS_EVENT_NODE_STRU), exception_node,
                           sizeof(DMS_EVENT_NODE_STRU));
...
static int dms_event_converge_resume_add_to_cb(struct dms_dev_ctrl_block *dev_cb, DMS_EVENT_NODE_STRU *exception_node)
{
    struct dms_converge_event_list *converge_list = NULL;
    ka_list_head_t *pos = NULL;
    ka_list_head_t *n = NULL;
...
static int dms_event_converge_occur_add_to_cb(struct dms_dev_ctrl_block *dev_cb, DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    if (memcpy_s((void *)&event_node->event, sizeof(struct dms_event_para), (void *)&exception_node->event,
                 sizeof(struct dms_event_para)) != 0) {
    ...
    converge_list->event_num++;
    converge_list->health_code = event_node->event.severity > converge_list->health_code ? event_node->event.severity :
                                                                                           converge_list->health_code;
...
int dms_get_event_code_from_event_cb(u32 devid, u32 *health_code, u32 health_len, struct shm_event_code *event_code,
                                     u32 event_len)
{
    struct dms_dev_ctrl_block *dev_cb = NULL;
    ka_list_head_t *pos = NULL;
    ...
    {
        event_node = ka_list_entry(pos, DMS_EVENT_NODE_STRU, node);
        health_code[0] = event_node->event.severity > health_code[0] ? event_node->event.severity : health_code[0];
        if (i < event_len) {
    ...
    if (i > event_len) {
        dms_warn("Event_list is larger than event_code array. (dev_id=%u; event_num=%u; max=%u)\n", devid, i,
                 event_len);
    }
    return DRV_ERROR_NONE;
}
...
int dms_event_id_to_error_string(u32 devid, u32 event_id, char *event_str, u32 name_len)
{
    ...
    if (name_len >= DMS_MAX_EVENT_NAME_LENGTH) {
        ret = snprintf_s(event_str, name_len, name_len - 1, "node type=%s, sensor type=%s, event state=%s", node_type,
                         sensor_type, event_state);
    } else {
        ret = snprintf_s(event_str, name_len, name_len - 1, "node type=0x%x, sensor type=0x%x, event=%u", dev_node_type,
                         dev_sensor_type, dev_event_state);
    }
...
int dms_event_converge_to_exception(struct dms_event_obj *event_obj, DMS_EVENT_NODE_STRU *exception_node)
{
    ...
    if ((event_obj == NULL) || (exception_node == NULL)) {
        dms_err("Invalid parameter. (event_obj=\"%s\"; exception_node=\"%s\")\n", (event_obj == NULL) ? "NULL" : "OK",
                (exception_node == NULL) ? "NULL" : "OK");
        return DRV_ERROR_PARA_ERROR;
    }

    (void)memset_s((void *)exception_node, sizeof(DMS_EVENT_NODE_STRU), 0, sizeof(DMS_EVENT_NODE_STRU));
    ...
    if (ret != 0) {
        dms_warn("Get event severity failed. (ret=%d)\n", ret);
    } else {
