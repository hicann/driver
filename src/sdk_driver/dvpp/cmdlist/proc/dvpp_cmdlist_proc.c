    ...
    for (i = 0; i < ctx->time_cost_stat.node_num; i++) {
        ...
        ka_hlist_for_each_entry_safe(pos, n, head, hlist)
        {
            ka_fs_seq_printf(m,
                             ...
                             "%18lld"
                             "\n",
                             pos->streamid, pos->taskid, pos->time_cost_usr_copy_to_kern, pos->time_cost_vmalloc,
                             pos->time_cost_dma_alloc, pos->time_cost_u2k_map, pos->time_cost_pcie_copy);
        }
    }
...
{
    ka_fs_seq_printf(m, "Welcome To Dvpp Cmdlist Proc.\n");
    disaply_time_cost(m);
    ka_fs_seq_printf(m, "\n");
...
static int32_t hanle_left_str_and_right_num(char *buf, char *target, uint64_t len, uint32_t *num)
{
    ...
    DVPP_CMDLIST_VPC_LOG_ERROR(
        "cmd is error, pls input 'echo enable_time_cost > /proc/dvpp_cmdlist' or "
...
static const ka_procfs_ops_t dvpp_cmdlist_proc_ops = {ka_fs_init_pf_open(dvpp_cmdlist_proc_open) ka_fs_init_pf_read(
    ka_fs_seq_read) ka_fs_init_pf_write(dvpp_cmdlist_proc_write) ka_fs_init_pf_lseek(ka_fs_seq_lseek)
