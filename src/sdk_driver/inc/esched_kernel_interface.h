...
struct sched_published_event_func {
    int (*event_ack_func)(unsigned int devid, unsigned int subevent_id, const char *msg, unsigned int msg_len,
                          void *priv);
...
int sched_query_curr_non_sched_event(unsigned int chip_id, struct sched_trace_input *para,
                                     struct sched_sync_event_trace *trace_result);
