...
STATIC int udis_check_input_valid(
    unsigned int udevid, UDIS_MODULE_TYPE module_type, const struct udis_dev_info *input_info)
{
    ...
    if (module_type < 0 || module_type >= UDIS_MODULE_MAX) {
        udis_err(
            "udis_info module_type invalid.(udevid=%u; module_type=%u; module_type_max=%d)\n", udevid, module_type,
            UDIS_MODULE_MAX);
        return -EINVAL;
    }
    ...
    if ((name_len == 0) || (name_len >= UDIS_MAX_NAME_LEN)) {
        udis_err(
            "name length is invalid. (udevid=%u; module_type=%u; name_len=%u; min_len=%u; max_len=%u)\n", udevid,
            module_type, name_len, 1, UDIS_MAX_NAME_LEN - 1);
        return -EINVAL;
    }

    if (input_info->data_len > UDIS_MAX_DATA_LEN) {
        udis_err(
            "udis_info data length is too large. (udevid=%u; module_type=%u; name=%s; data_len=%u)\n", udevid,
            module_type, input_info->name, input_info->data_len);
        return -EINVAL;
    }
    return 0;
}
...
STATIC struct udis_info_stu *udis_find_info_block(
    const struct udis_ctrl_block *ucb, UDIS_MODULE_TYPE module_type, const char *name, enum udis_search_scope uss)
{
...
STATIC int udis_check_info_block_valid(u32 udevid, const struct udis_info_stu *info_block)
{
    ...
    switch (info_block->update_type) {
        ...
        default:
            ...
            udis_err(
                "udis update_type check invalid. (udevid=%u; update_type=%u; ret=%d)\n", udevid,
                info_block->update_type, ret);
    }
...
STATIC int udis_store_info(const struct udis_dev_info *src_info, struct udis_info_stu *dst_info_block)
{
    ...
    if ((src_info->update_type < 0) || (src_info->update_type >= UPDATE_TYPE_MAX)) {
        udis_err(
            "udis update_type check invalid, should be in [0, %u). (update_type=%u; ret=%d)\n", UPDATE_TYPE_MAX,
    ...
    if (src_info->last_update_time > 0) {
        info_block_tmp.last_update_time = src_info->last_update_time;
    } else {
...
STATIC int udis_load_info_block(
    u32 udevid, UDIS_MODULE_TYPE module_type, const struct udis_info_stu *src_info_block,
    struct udis_dev_info *dst_info)
{
    int ret = 0;
    ...
    if (ret != 0) {
        ret = -EIO;
        udis_err(
            "udis dst_info memcpy_s error. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
            src_info_block->name, ret);
        return ret;
    }
...
STATIC int udis_update_load_info_block(
    struct udis_ctrl_block *ucb, u32 udevid, UDIS_MODULE_TYPE module_type, struct udis_dev_info *info_request)
{
    int ret = 0;
    struct udis_info_stu *info_block = NULL;
    ...
    if (ret != 0) {
        udis_err(
            "udis_update_info failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
            info_request->name, ret);
        return ret;
    }
    ...
    if (info_block == NULL) {
        ret = -ENODATA;
        udis_err(
            "This module has no such name. (udevid=%u; module_type=%u; name=%s)\n", udevid, module_type,
            info_request->name);
        goto out;
    }
    ...
    if (ret != UDIS_DATA_VALID && ret != UDIS_DATA_UPDATE_IMMEDIATELY) {
        ret = -EINVAL;
        udis_err(
            "udis info_block's data is invalid. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
            info_block->name, ret);
        goto out;
    }
    ...
    if (ret != 0) {
        udis_err(
            "udis_load_info_block failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
            info_request->name, ret);
        goto out;
    }
out:
...
int hal_kernel_set_udis_info(
    unsigned int udevid, UDIS_MODULE_TYPE module_type, const struct udis_dev_info *udis_set_info)
{
    int ret = 0;
    ...
    if (info_block == NULL) {
        ...
        if (info_block == NULL) {
            ret = -ENOSPC;
            udis_err(
                "No empty block for new name. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
                udis_set_info->name, ret);
    ...
    if (ret != 0) {
        udis_err(
            "udis_store_info failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, module_type,
            udis_set_info->name, ret);
    }
...
int hal_kernel_get_udis_info(unsigned int udevid, UDIS_MODULE_TYPE module_type, struct udis_dev_info *info_request)
{
    ...
    if (info_block == NULL) {
        ret = -ENODATA;
        udis_debug(
            "This module has no such name. (udevid=%u; type=%u; name=%s; ret=%d)\n", udevid, module_type,
            info_request->name, ret);
    ...
    if (ret != 0) {
        udis_ex_notsupport_err(
            ret, "Access identify failed. (udevid=%u; type=%u; name=%s; ret=%d; acc_ctrl=0x%x)\n", udevid, module_type,
            info_request->name, ret, info_block->acc_ctrl);
        goto unlock_udis_info;
    }
...
STATIC int udis_fill_info_block(
    unsigned int udevid, const struct udis_node *addr_node, struct udis_info_stu *info_block)
{
    int ret;
    ...
    if (ret != 0) {
        udis_err(
            "strncpy_s failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, addr_node->module_type,
            addr_node->name, ret);
        return -ENOMEM;
    }
...
STATIC ka_dma_addr_t udis_get_info_block_addr(
    unsigned int udevid, const struct udis_ctrl_block *udis_cb, UDIS_MODULE_TYPE module_type,
    const struct udis_info_stu *info_block)
{
    ...
    if (offset >= (unsigned long)(void *)info_block) {
        udis_err(
            "Offset overflow. (udevid=%u; module_type=%u; name=%s; offset=%lu)\n", udevid, module_type,
            info_block->name, offset);
    ...
    if ((devdrv_get_connect_protocol(udevid)) == CONNECT_PROTOCOL_UB) {
        ...
        if (info_va_addr <= udis_va_base_addr) {
            udis_err(
                "Info host va addr overflow. (udevid=%u; module_type=%u; name=%s)\n", udevid, module_type,
                info_block->name);
        ...
        if (info_dma_addr <= udis_dma_base_addr) {
            udis_err(
                "Info host dma addr overflow. (udevid=%u; module_type=%u; name=%s)\n", udevid, module_type,
                info_block->name);
            return UDIS_BAD_DMA_ADDR;
        }
...
STATIC struct udis_info_stu *udis_dma_remap_info_block(
    unsigned int udevid, const struct udis_ctrl_block *udis_cb, ka_dma_addr_t host_addr)
{
    unsigned long offset = 0;
...
STATIC int udis_alloc_discrete_info_block(
    unsigned int udevid, const struct udis_node *addr_node, ka_dma_addr_t *host_addr)
{
    int ret;
    ...
    if (info_block == NULL) {
        ...
        udis_err(
            "Get unused info block failed. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type,
            addr_node->name);
        return -ENOSPC;
    }
    ...
    if (info_addr == UDIS_BAD_DMA_ADDR) {
        ka_task_up_write(&udis_cb->udis_info_lock);
        udis_err(
            "Failed to get addr. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type,
            addr_node->name);
        return -EFAULT;
    }
    ...
    *host_addr = info_addr;
    ...
    if (ret != 0) {
        ka_task_up_write(&udis_cb->udis_info_lock);
        udis_err(
            "Failed to fill info block's name. (udevid=%u; module_type=%u; name=%s, ret=%d)\n", udevid,
...
STATIC void udis_clear_unified_info_data_len(
    unsigned int udevid, struct udis_ctrl_block *udis_cb, UDIS_MODULE_TYPE module_type)
{
    int i;
    struct udis_info_stu *module_base_addr = NULL;
    struct udis_info_stu *info_block = NULL;
...
int udis_alloc_info_block(unsigned int udevid, const struct udis_node *addr_node, ka_dma_addr_t *host_addr)
{
    if (host_addr == NULL) {
        udis_err(
            "Invalid param. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type, addr_node->name);
        return -EINVAL;
    }
...
void udis_free_info_block(unsigned int udevid, struct udis_node *addr_node)
{
    ...
    if ((devdrv_get_connect_protocol(udevid)) == CONNECT_PROTOCOL_UB) {
        if ((addr_node->host_va_addr <
             (va_addr_t)(uintptr_t)(udis_cb->udis_info_buf) + UDIS_MODULE_OFFSET * addr_node->module_type) ||
            (addr_node->host_va_addr >= (va_addr_t)(uintptr_t)(udis_cb->udis_info_buf) +
                                            UDIS_MODULE_OFFSET * addr_node->module_type + UDIS_MODULE_OFFSET)) {
            udis_err(
                "Invalid host_va_addr. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type,
                addr_node->name);
            return;
        }
    } else {
        if ((addr_node->host_dma_addr < udis_cb->udis_info_buf_dma + UDIS_MODULE_OFFSET * addr_node->module_type) ||
            (addr_node->host_dma_addr >=
             udis_cb->udis_info_buf_dma + UDIS_MODULE_OFFSET * addr_node->module_type + UDIS_MODULE_OFFSET)) {
            udis_err(
                "Invalid host_dma_addr. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type,
                addr_node->name);
            return;
        }
    }
    ...
    if (ka_base_strcmp(addr_node->name, UDIS_UNIFIED_MODULE_INFO) == 0) {
        ...
        if (ret != 0) {
            udis_err(
                "Clear host unified info failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid,
        ...
        if (ret != 0) {
            udis_err(
                "Clear block name failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid,
                addr_node->module_type, addr_node->name, ret);
        }
    }
...
STATIC void udis_dma_fail_process(unsigned int udevid, struct udis_ctrl_block *udis_cb, const struct udis_node *node)
{
    ...
    if (ka_base_strcmp(node->name, UDIS_UNIFIED_MODULE_INFO) == 0) {
        ...
        if (ret != 0) {
            udis_err(
                "Clear block len failed. (udevid=%u; module=%u; name=%s; ret=%d)", udevid, node->module_type,
                node->name, ret);
        }
    }
...
static struct devdrv_urma_copy *udis_alloc_devdrv_urma_copy(unsigned int udevid, u64 len, u64 offset, void *seg)
{
    ...
    urma_copy =
        (struct devdrv_urma_copy *)dbl_kzalloc(sizeof(struct devdrv_urma_copy), KA_GFP_KERNEL | __KA_GFP_ACCOUNT);
    if (urma_copy == NULL) {
...
int udis_sync_copy(unsigned int udevid, struct udis_ctrl_block *udis_cb, const struct udis_node *node)
{
    ...
    if ((devdrv_get_connect_protocol(udevid) == CONNECT_PROTOCOL_UB)) {
        host_urma_copy = udis_alloc_devdrv_urma_copy(
            udevid, node->data_len, node->host_va_addr - (va_addr_t)(uintptr_t)(udis_cb->udis_info_buf),
            node->host_segment);
        ...
        device_urma_copy =
            udis_alloc_devdrv_urma_copy(udevid, node->data_len, node->device_addr_offset, node->device_segment_import);
        ...
        ret = hal_kernel_devdrv_dma_sync_copy(
            udevid, DEVDRV_DMA_DATA_COMMON, node->dev_dma_addr, node->host_dma_addr, node->data_len,
    ...
    if (ret != 0) {
        ...
        ka_task_up_write(&udis_cb->udis_info_lock);
        udis_err(
            "Copy failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, node->module_type, node->name, ret);
        goto free_urma_copy;
    }
    ka_task_up_write(&udis_cb->udis_info_lock);
...
STATIC struct udis_node *udis_find_addr_node(
    struct udis_ctrl_block *udis_cb, UDIS_MODULE_TYPE module_type, const char *name)
{
...
STATIC int udis_link_ub_copy(
    unsigned int udevid, struct udis_link_ub_node *ub_nodes, struct udis_ctrl_block *udis_cb, unsigned int node_num)
{
    int ret;
    int i = 0;
    struct devdrv_urma_copy *host_urma_copy = NULL;
    ...
    for (; i < node_num; i++) {
        host_urma_copy = udis_alloc_devdrv_urma_copy(
            udevid, ub_nodes[i].data_len, ub_nodes[i].host_va_addr - (va_addr_t)(uintptr_t)(udis_cb->udis_info_buf),
            ub_nodes[i].host_segment);
        if (host_urma_copy == NULL) {
            udis_err("udis_alloc_devdrv_urma_copy host failed. (udevid=%u)\n", udevid);
            return -EINVAL;
        }

        device_urma_copy = udis_alloc_devdrv_urma_copy(
            udevid, ub_nodes[i].data_len, ub_nodes[i].device_addr_offset, ub_nodes[i].device_segment_import);
        if (device_urma_copy == NULL) {
            udis_free_devdrv_urma_copy(host_urma_copy);
            udis_err("udis_get_devdrv_urma_copy device failed. (udevid=%u)\n", udevid);
            return -EINVAL;
...
STATIC int udis_link_copy(unsigned int udevid, struct udis_ctrl_block *udis_cb, UDIS_UPDATE_TYPE addr_list_type)
{
    int ret;
    ...
    if ((devdrv_get_connect_protocol(udevid) == CONNECT_PROTOCOL_UB)) {
        ...
        ret = hal_kernel_devdrv_dma_sync_link_copy(
            udevid, DEVDRV_DMA_DATA_COMMON, DEVDRV_DMA_WAIT_INTR, dma_nodes, node_num);
    }
    if (ret != 0) {
        if (ret != -ENODEV) {
...
unsigned long long udis_get_link_timestamp(unsigned int udevid) { return g_link_timestamp[udevid]; }
