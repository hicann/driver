...
struct devdrv_common_msg_client *udis_get_common_msg_client(void) { return &g_udis_common_chan; }
...
int udis_link_nodes_init(unsigned int udevid)
{
    ...
    if ((devdrv_get_connect_protocol(udevid) == CONNECT_PROTOCOL_UB)) {
        ...
        link_nodes->node.ub_nodes = (struct udis_link_ub_node *)dbl_kzalloc(
            (sizeof(struct udis_link_ub_node) * UDIS_NODE_NUM_INIT), KA_GFP_KERNEL | __KA_GFP_ACCOUNT);
        if (link_nodes->node.ub_nodes == NULL) {
        ...
        link_nodes->node.dma_nodes = (struct devdrv_dma_node *)dbl_kzalloc(
            (sizeof(struct devdrv_dma_node) * UDIS_NODE_NUM_INIT), KA_GFP_KERNEL | __KA_GFP_ACCOUNT);
        if (link_nodes->node.dma_nodes == NULL) {
...
STATIC int udis_ub_nodes_scale_up(unsigned int udevid, struct udis_link_nodes *link_nodes)
{
    ...
    new_nodes = (struct udis_link_ub_node *)dbl_kzalloc(
        sizeof(struct udis_link_ub_node) * new_capacity, KA_GFP_KERNEL | __KA_GFP_ACCOUNT);
    if (new_nodes == NULL) {
    ...
    ret = memcpy_s(
        new_nodes, sizeof(struct udis_link_ub_node) * old_capacity, old_nodes,
        sizeof(struct udis_link_ub_node) * old_capacity);
    if (ret != 0) {
        dbl_kfree(new_nodes);
        new_nodes = NULL;
...
STATIC int udis_dma_nodes_scale_up(unsigned int udevid, struct udis_link_nodes *link_nodes)
{
    ...
    new_nodes = (struct devdrv_dma_node *)dbl_kzalloc(
        sizeof(struct devdrv_dma_node) * new_capacity, KA_GFP_KERNEL | __KA_GFP_ACCOUNT);
    if (new_nodes == NULL) {
    ...
    ret = memcpy_s(
        new_nodes, sizeof(struct devdrv_dma_node) * old_capacity, old_nodes,
        sizeof(struct devdrv_dma_node) * old_capacity);
    if (ret != 0) {
        dbl_kfree(new_nodes);
        new_nodes = NULL;
        udis_err("memcpy_s failed. (udevid=%u; ret=%d)\n", udevid, ret);
...
STATIC int udis_update_ub_nodes(
    unsigned int udevid, const struct udis_ctrl_block *udis_cb, UDIS_UPDATE_TYPE update_type,
    struct udis_link_nodes *link_nodes)
{
    int ret;
    ...
    ka_list_for_each_entry(cur, &udis_cb->addr_list[update_type], list)
    {
        ...
        link_nodes->node.ub_nodes[node_num].device_addr_offset = cur->device_addr_offset;
        ++node_num;
...
STATIC int udis_update_dma_nodes(
    unsigned int udevid, const struct udis_ctrl_block *udis_cb, UDIS_UPDATE_TYPE update_type,
    struct udis_link_nodes *link_nodes)
{
    int ret;
...
STATIC int udis_update_link_nodes(
    unsigned int udevid, const struct udis_ctrl_block *udis_cb, UDIS_UPDATE_TYPE update_type)
{
    int ret = 0;
    unsigned int node_num = 0;
    ...
    if (update_type < UPDATE_PERIOD_LEVEL_1) {
        udis_info(
            "Not period update type, no need to flush link ub nodes. (udevid=%u; update_type=%u; node_num=%u)\n",
...
STATIC int udis_check_addr_node(const struct udis_node *addr_node)
{
    ...
    if (addr_node->module_type >= UDIS_MODULE_MAX) {
        udis_err(
            "Invalid module_type. (module_type=%u; max_module_type=%u)\n", addr_node->module_type, UDIS_MODULE_MAX - 1);
        return -EINVAL;
    }
    ...
    if ((name_len == 0) || (name_len >= UDIS_MAX_NAME_LEN)) {
        udis_err(
            "Invalid name_len. (module_type=%u; name_len=%u; max_name_len=%u)\n", addr_node->module_type, name_len,
            UDIS_MAX_NAME_LEN - 1);
        return -EINVAL;
    }
    ...
    if (addr_node->update_type >= UPDATE_TYPE_MAX) {
        udis_err(
            "Invalid update type. (mode_type=%u; name=%s; update_type=%u; max_update_type=%u)\n",
    ...
    if (((addr_node->dev_dma_addr == UDIS_BAD_DMA_ADDR) && (addr_node->dev_va_addr == UDIS_BAD_VA_ADDR)) ||
        (addr_node->data_len == 0) || ((is_discrete_info != 0) && (addr_node->data_len > UDIS_MAX_DATA_LEN)) ||
        ((is_discrete_info == 0) && (addr_node->data_len != UDIS_UNIFIED_MODULE_OFFSET))) {
        udis_err(
            "Invalid param. (module_type=%u; name=%s; dma_addr_is_bad=%d; data_len=%u)\n", addr_node->module_type,
            addr_node->name, addr_node->dev_dma_addr == UDIS_BAD_DMA_ADDR, addr_node->data_len);
        return -EINVAL;
    }
...
STATIC struct devdrv_seg_info *udis_alloc_seg_info(
    unsigned int udevid, va_addr_t addr, unsigned int data_len, u32 token_value)
{
...
STATIC int udis_get_segment(unsigned int udevid, struct udis_ctrl_block *udis_cb, struct udis_node *addr_node)
{
    ...
    device_segment_import = devdrv_import_seg(
        udevid, addr_node->token_value, (void *)addr_node->device_segment, addr_node->device_segment_len,
        &device_segment_import_len);
    ...
    seg_info = udis_alloc_seg_info(
        udevid, (va_addr_t)(uintptr_t)(udis_cb->udis_info_buf), UDIS_MODULE_OFFSET * UDIS_MODULE_MAX, token_value);
    if (seg_info == NULL) {
...
STATIC int udis_addr_node_register(unsigned int udevid, struct udis_ctrl_block *udis_cb, struct udis_node *addr_node)
{
    int ret;
    ...
    if (ret != 0) {
        udis_err(
            "Init info block failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, addr_node->module_type,
            addr_node->name, ret);
        return ret;
    }
    if ((devdrv_get_connect_protocol(udevid) == CONNECT_PROTOCOL_UB)) {
    ...
    if ((devdrv_get_connect_protocol(udevid)) == CONNECT_PROTOCOL_UB) {
        ...
        if (ret != 0) {
            udis_err(
                "udis_get_segment failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid,
    ...
    if (ret != 0) {
        udis_err(
            "Add node to addr list failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid,
            addr_node->module_type, addr_node->name, ret);
        goto free_info_block;
    }
    ...
    if (ret != 0) {
        udis_err(
            "Update link nodes failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, addr_node->module_type,
            addr_node->name, ret);
        goto remove_node;
    }
    return 0;
...
STATIC int udis_register_addr(void *msg, u32 *ack_len)
{
    ...
    if (repeat_node != NULL) {
        udis_info(
            "Already have the same addr node. (udevid=%u; module_type=%u; name=%s)\n", udevid, addr_node->module_type,
            addr_node->name);
        *ack_len = 0;
        ret = 0;
    ...
    if (ret != 0) {
        udis_err(
            "Add new addr node failed. (udevid=%u; module_type=%u; name=%s; ret=%d)\n", udevid, addr_node->module_type,
            addr_node->name, ret);
        goto unlock_addr_list_lock;
    }
    ...
    *ack_len = 0;
    udis_info(
        "Register addr node success.\
 (udevid=%u; module_type=%u; name=%s; update_type=%u; acc_ctrl=%u; data_len=%u)\n",
        udevid, addr_node->module_type, addr_node->name, addr_node->update_type, addr_node->acc_ctrl,
        addr_node->data_len);
...
STATIC int udis_rx_common_msg_process(
    unsigned int dev_id, void *data, unsigned int in_data_len, unsigned int out_data_len, unsigned int *real_out_len)
{
    unsigned int msg_id;
    struct udis_msg_info *msg_info = NULL;
    ...
    if ((dev_id >= UDIS_DEVICE_UDEVID_MAX) || (data == NULL) || (real_out_len == NULL) ||
        ...
        udis_err(
            "date(%pK) or real_out_len(%pK) is NULL. (devid=%u; in_data_len=%u)\n", data, real_out_len, dev_id,
            in_data_len);
        return -EINVAL;
    }
...
int udis_send_host_vf_uninit_notify(unsigned int udevid)
{
    ...
    ret = devdrv_common_msg_send(
        udevid, &udis_msg_info, sizeof(udis_msg_info), sizeof(udis_msg_info), &out_len, DEVDRV_COMMON_MSG_UDIS);
    if (ret != 0) {
