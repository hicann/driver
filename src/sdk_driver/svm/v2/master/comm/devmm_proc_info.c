...
int devmm_remap_huge_pages(struct devmm_svm_process *svm_proc, u64 va, ka_page_t **hpages, u64 pg_num, u32 pg_prot)
{
    ...
    for (i = 0; i < pg_num; i++) {
        ret = ka_mm_remap_pfn_range(vma, va + SVM_MASTER_HUGE_PAGE_SIZE * i, ka_mm_page_to_pfn(hpages[i]),
                                    SVM_MASTER_HUGE_PAGE_SIZE, devmm_make_pgprot(pg_prot, false));
        if (ret != 0) {
...
int devmm_remap_giant_pages(struct devmm_svm_process *svm_proc, u64 va, ka_page_t **hpages, u64 pg_num, u32 pg_prot,
                            bool has_interval)
{
    ka_vm_area_struct_t *vma = NULL;
    ...
    for (i = 0; i < pg_num; i++) {
        ret = ka_mm_remap_pfn_range(vma, va + SVM_MASTER_GIANT_PAGE_SIZE * i, ka_mm_page_to_pfn(hpages[i]),
                                    SVM_MASTER_GIANT_PAGE_SIZE, devmm_make_pgprot(pg_prot, false));
        if (ret != 0) {
            devmm_drv_err("Vm_insert_page failed. (ret=%d; va=0x%llx; i=%llu; page_num=%llu)\n", ret, va, i, pg_num);
...
void devmm_svm_set_mapped_with_heap(struct devmm_svm_process *svm_process, unsigned long va, size_t size, u32 logic_id,
...
void devmm_svm_clear_mapped_with_heap(struct devmm_svm_process *svm_process, unsigned long va, size_t size, u32 devid,
                                      struct devmm_svm_heap *heap)
{
    u32 *page_bitmap = NULL;
...
static int devmm_dev_page_fault_check_va_bitmap(struct devmm_svm_process *svm_proc, u32 *page_bitmap, u64 va)
{
    ...
    int ret = ((devmm_page_bitmap_is_page_available(page_bitmap) == 0) ||
               devmm_page_bitmap_is_locked_host(page_bitmap) || devmm_page_bitmap_is_ipc_open_mem(page_bitmap) ||
...
STATIC int devmm_setup_update_other_device_heap(struct devmm_svm_process *svm_pro,
                                                struct devmm_chan_setup_device *chan_setup, u32 devid, u32 updated_idx,
                                                u32 heap_cnt)
{
...
static int _devmm_get_svm_mem_attrs(struct devmm_svm_process *svm_proc, struct devmm_svm_heap *heap, u32 *bitmap,
                                    u64 addr, struct devmm_memory_attributes *attr)
{
    *attr = (struct devmm_memory_attributes){0};
...
#ifndef UVM_OPEN
static int _devmm_get_uvm_mem_attrs(struct devmm_svm_process *svm_proc, struct devmm_uvm_heap *heap,
                                    struct uvm_page_info *page_info, u64 addr, struct devmm_memory_attributes *attr)
{
    *attr = (struct devmm_memory_attributes){0};
...
int devmm_get_soma_mem_attrs(struct devmm_svm_process *svm_proc, u64 addr, struct devmm_memory_attributes *attr)
{
    ...
    if ((svm_proc == NULL) || (attr == NULL)) {
        devmm_drv_err("Invalid soma attr context. (svm_proc_is_null=%d; attr_is_null=%d)\n", (svm_proc == NULL),
                      (attr == NULL));
        return -EINVAL;
    }
#endif
...
bool devmm_acquire_aligned_addr_and_cnt(u64 address, u64 byte_count, int is_svm_huge, u64 *aligned_down_addr,
                                        u64 *aligned_count)
{
    u64 aligned_up_addr;
...
int devmm_insert_host_page_range(struct devmm_svm_process *svm_pro, u64 dst, u64 byte_count,
    ...
    for (i = 0; i < pg_num; i++) {
        ...
        if (ret != 0) {
            devmm_drv_err("ka_mm_remap_pfn_range failed. (va=0x%llx; i=%llu; hostpid=%d; devid=%d; vfid=%d)\n",
                          tmp_vaddr, i, svm_proc->process_id.hostpid, svm_proc->process_id.devid,
                          svm_proc->process_id.vfid);
            goto clear_pfn_range;
        }
...
{
    return (attr->is_uvm_host || attr->is_svm_host || attr->is_local_host || attr->is_host_pin ||
            attr->is_svm_host_agent);
}

bool devmm_is_device_agent(struct devmm_memory_attributes *attr)
{
    return ((attr->is_svm_device && (!devmm_is_host_agent(attr->devid))) ||
    ...
    for (i = 0; i < SVM_MAX_AGENT_NUM; i++) {
        ...
        if (ret != 0) {
            ...
            devmm_drv_warn("Free share page. (devid=%d; ret=%d; va=0x%llx; real_size=%llu)\n", free_info->head.dev_id,
                           ret, free_info->va, free_info->real_size);
#endif
            continue;
        }
    }
...
STATIC void devmm_fill_free_pages_info(u64 va, struct devmm_devid *svmid, struct devmm_svm_heap *heap,
                                       struct devmm_free_dev_mem_info *dev_mem_info,
                                       struct devmm_chan_free_pages *free_info)
{
    free_info->va = ka_base_round_down(va, heap->chunk_page_size);
    free_info->real_size = ka_base_max((dev_mem_info->last_devshared_index - dev_mem_info->first_devshared_index + 1),
...
STATIC bool devmm_is_need_send_free_msg(struct devmm_free_dev_mem_info *dev_mem_info, u64 cur_page_num,
                                        u64 max_page_num)
{
...
STATIC int devmm_chan_send_msg_soma_free_pages(struct devmm_svm_process *svm_pro, u32 devid, uint64_t va)
{
    ...
    ret = devmm_chan_msg_send(&para, sizeof(struct devmm_chan_soma_free_sync),
                              sizeof(struct devmm_chan_soma_free_sync));
    if (ret != 0) {
    ...
    for (j = 0, free_va = va, send_va = va; j < page_num; j++, free_va += heap->chunk_page_size) {
        ...
        if (devmm_is_need_send_free_msg(&dev_mem_info, j, page_num - 1)) {
            ...
            if (free_info.head.dev_id < SVM_MAX_AGENT_NUM) {
                devmm_free_pages_cache(svm_pro, free_info.head.logical_devid, (u32)page_num, heap->chunk_page_size,
                                       free_info.va, reuse);
            }
...
int devmm_notify_deviceprocess(struct devmm_svm_process *svm_proc)
{
    ...
    for (i = 0; i < SVM_MAX_AGENT_NUM; i++) {
        ...
        devmm_drv_debug("Show details. "
                        ...
                        svm_proc->process_id.hostpid, svm_proc->process_id.devid, svm_proc->process_id.vfid, i, phyid,
                        vfid, devmm_device_is_ready(phyid));
...
STATIC void devmm_update_heap_broadcast(struct devmm_svm_process *svm_pro, struct devmm_chan_update_heap *update_heap)
{
    ...
    for (i = 0; i < (u32)SVM_MAX_AGENT_NUM; i++) {
        ...
        ret = devmm_chan_msg_send(update_heap, sizeof(struct devmm_chan_update_heap),
                                  sizeof(struct devmm_chan_msg_head));
        if (ret != 0) {
#ifndef EMU_ST
...
static int devmm_destory_svm_heap_bitmap(struct devmm_svm_process *svm_pro, struct devmm_svm_heap *heap)
{
    ...
    for (i = 0; i < page_cnt; i += page_num) {
        ...
        if (ret != 0) {
            devmm_drv_err("Va heap_start_addr can't free, is being used by RUNTIME, must call release api first. "
...
STATIC int devmm_ioctl_update_heap(struct devmm_svm_process *svm_pro, struct devmm_ioctl_arg *arg)
{
    ...
    devmm_drv_debug("Show details. "
                    ...
                    cmd->op, cmd->heap_type, cmd->heap_idx, cmd->heap_size, svm_pro->process_id.hostpid,
                    svm_pro->process_id.devid, svm_pro->process_id.vfid);
...
void devmm_notify_wait_device_close_process(struct devmm_svm_process *svm_proc, u32 logical_devid, u32 phy_devid,
                                            u32 vfid)
{
    int ret, i;
...
STATIC int devmm_ioctl_close_device(struct devmm_svm_process *svm_proc, struct devmm_ioctl_arg *arg)
{
    ...
    devmm_drv_run_info("Close logical_device. (logical_devid=%d; phyid=%d; vfid=%d; hostpid=%d; devpid=%d)\n",
                       arg->head.logical_devid, arg->head.devid, arg->head.vfid, svm_proc->process_id.hostpid,
                       svm_proc->deviceinfo[logical_devid].devpid);
...
struct devmm_ioctl_handlers_st devmm_ioctl_handlers[DEVMM_SVM_CMD_MAX_CMD] = {
    ...
    [_KA_IOC_NR(DEVMM_SVM_DESTROY_ADDR)] = {devmm_ioctl_destroy_addr,
                                            DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    ...
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_CLOSE)] = {devmm_ioctl_ipc_mem_close, DEVMM_ADD_SUB_REF | DEVMM_CMD_NOT_SURPORT_VDEV |
                                                                            DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_CREATE)] = {devmm_ioctl_ipc_mem_create, DEVMM_ADD_SUB_REF |
                                                                              DEVMM_CMD_NOT_SURPORT_VDEV |
                                                                              DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_SET_PID)] = {devmm_ioctl_ipc_mem_set_pid,
                                               DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_DESTROY)] = {devmm_ioctl_ipc_mem_destroy,
                                               DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_QUERY)] = {devmm_ioctl_ipc_mem_query,
                                             DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_SET_ATTR)] = {devmm_ioctl_ipc_set_attr,
                                                DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_IPC_MEM_GET_ATTR)] = {devmm_ioctl_ipc_get_attr,
                                                DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    ...
    [_KA_IOC_NR(DEVMM_SVM_MEM_REMOTE_MAP)] = {devmm_ioctl_mem_remote_map,
                                              DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_VDEV |
                                                  DEVMM_CMD_NOT_SURPORT_HOST_AGENT | DEVMM_ADD_REF},
    [_KA_IOC_NR(DEVMM_SVM_MEM_REMOTE_UNMAP)] = {devmm_ioctl_mem_remote_unmap,
                                                DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_VDEV |
                                                    DEVMM_CMD_NOT_SURPORT_HOST_AGENT | DEVMM_OPS_SUCCESS_SUB_REF},
    [_KA_IOC_NR(DEVMM_SVM_DBG_VA_STATUS)] = {devmm_ioctl_get_va_status_info, 0},
    [_KA_IOC_NR(DEVMM_SVM_MAP_DEV_RESERVE)] = {devmm_ioctl_map_dev_reserve,
                                               DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_PROCESS_STATUS_QUERY)] = {devmm_ioctl_query_process_status,
                                                    DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
#ifndef UVM_OPEN
    ...
    [_KA_IOC_NR(DEVMM_SVM_QUERY_MEM_USEDINFO)] = {devmm_ioctl_query_device_mem_info,
                                                  DEVMM_CONVERT_ID | DEVMM_CMD_NOLOCK |
                                                      DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_CHECK_MEMINFO)] = {devmm_ioctl_check_meminfo,
                                             DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    ...
    [_KA_IOC_NR(DEVMM_SVM_MEM_MAP)] = {devmm_ioctl_mem_map,
                                       DEVMM_CONVERT_ID | DEVMM_ADD_REF | DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_UNMAP)] = {devmm_ioctl_mem_unmap,
                                         DEVMM_CONVERT_ID | DEVMM_OPS_SUCCESS_SUB_REF | DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_QUERY_OWNER)] = {devmm_ioctl_mem_query_owner,
                                               DEVMM_CONVERT_ID | DEVMM_CMD_SURPORT_HOST_ID},
    ...
    [_KA_IOC_NR(DEVMM_SVM_MEM_IMPORT)] = {devmm_ioctl_mem_import, DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                                                      DEVMM_CMD_NOT_SURPORT_VDEV |
                                                                      DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_SET_PID)] = {devmm_ioctl_mem_set_pid,
                                           DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                               DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_SET_ATTR)] = {devmm_ioctl_mem_set_attr,
                                            DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                                DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_GET_ATTR)] = {devmm_ioctl_mem_get_attr,
                                            DEVMM_CMD_NOT_SURPORT_HOST_AGENT | DEVMM_CMD_NOT_SURPORT_VDEV},
    [_KA_IOC_NR(DEVMM_SVM_MEM_GET_INFO)] = {devmm_ioctl_mem_get_info,
                                            DEVMM_CMD_NOT_SURPORT_HOST_AGENT | DEVMM_CMD_NOT_SURPORT_VDEV},
    [_KA_IOC_NR(DEVMM_SVM_REGISTER_DMA)] = {devmm_ioctl_register_dma, DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_VDEV |
                                                                          DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                                                          DEVMM_ADD_SUB_REF},
    [_KA_IOC_NR(DEVMM_SVM_UNREGISTER_DMA)] = {devmm_ioctl_unregister_dma, DEVMM_CONVERT_ID |
                                                                              DEVMM_CMD_NOT_SURPORT_VDEV |
                                                                              DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_MEM_REPLAIR)] = {devmm_ioctl_mem_repair,
                                           DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                               DEVMM_CONVERT_ID | DEVMM_ADD_SUB_REF | DEVMM_CMD_WLOCK},
    [_KA_IOC_NR(DEVMM_SVM_RESERVE_ADDR_INFO_QUERY)] = {devmm_ioctl_resv_addr_info_query, DEVMM_ADD_SUB_REF},
    [_KA_IOC_NR(DEVMM_SVM_PREPARE_CLOSE_DEVICE)] = {devmm_ioctl_prepare_close_device,
                                                    DEVMM_CONVERT_ID | DEVMM_CMD_WLOCK},
    [_KA_IOC_NR(DEVMM_SVM_MEMCPY_BATCH)] = {devmm_ioctl_memcpy_batch,
                                            DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_MEM_MAP_CAP)] = {devmm_ioctl_mem_map_capability,
                                           DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_VDEV},
    [_KA_IOC_NR(DEVMM_SVM_SOMA_POOL_CREATE)] = {devmm_ioctl_soma_create_pool,
                                                DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_SOMA_POOL_DESTROY)] = {devmm_ioctl_soma_destroy_pool,
                                                 DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_SOMA_POOL_TRIM)] = {devmm_ioctl_soma_trim_pool,
                                              DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT},
    [_KA_IOC_NR(DEVMM_SVM_MEM_HANDLE_SET_ATTR)] = {devmm_ioctl_mem_handle_set_attr,
                                                   DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                                       DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_SURPORT_HOST_ID},
    [_KA_IOC_NR(DEVMM_SVM_MEM_HANDLE_GET_ATTR)] = {devmm_ioctl_mem_handle_get_attr,
                                                   DEVMM_CONVERT_ID | DEVMM_CMD_NOT_SURPORT_HOST_AGENT |
                                                       DEVMM_CMD_NOT_SURPORT_VDEV | DEVMM_CMD_SURPORT_HOST_ID},
};
...
err_exit:
    ...
    devmm_drv_err("Va error, please check input argument. (va=%llx; ref_flag=%x; page_bitmap=0x%x; ref=0x%x)\n", va,
                  ref_flag, heap->page_bitmap[pfn], heap->ref[pfn]);
    return NULL;
}
...
static int _devmm_set_page_ref_before_ioctl(struct devmm_svm_process *svm_proc, u32 cmd_flag,
                                            struct devmm_ioctl_addr_info *info, u32 index)
{
    if ((cmd_flag & DEVMM_ADD_REF) == DEVMM_ADD_REF) {
        ...
        return devmm_set_page_ref_of_adivse(svm_proc, info->heap[index], info->ref[index], info->va[index],
                                            info->size[index]);
    } else {
        return 0;
    }
                                              ...
                                              struct devmm_ioctl_addr_info *info, u32 index)
{
    if ((cmd_flag & DEVMM_SUB_REF) == DEVMM_SUB_REF) {
        ...
        devmm_clear_page_ref_of_advise(svm_proc, info->heap[index], info->ref[index], info->va[index],
                                       info->size[index]);
...
static void devmm_oper_page_ref_err_dfx(struct devmm_svm_process *svm_proc, struct devmm_heap_ref *ref, u32 ref_flag,
                                        u64 va)
{
    bool convert, async;
...
void devmm_clear_page_ref_after_ioctl(struct devmm_svm_process *svm_proc, u32 cmd_flag, int ret,
                                      struct devmm_ioctl_addr_info *addr_info)
{
    u32 i;
...
int devmm_chan_update_msg_logic_id(struct devmm_svm_process *svm_proc, struct devmm_chan_msg_head *msg_head)
{
    ...
    if (logical_devid >= SVM_MAX_AGENT_NUM) {
        devmm_drv_err("Get logic_id failed. (phyid=%u; vfid=%u; logicid=%u; msg_id=%u)\n", msg_head->dev_id,
                      msg_head->vfid, logical_devid, msg_head->msg_id);
        return -ENODEV;
    }
