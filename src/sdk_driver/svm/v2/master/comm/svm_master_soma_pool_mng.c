...
static int devmm_soma_pool_check_proc_and_dev(struct devmm_svm_process *svm_proc, const struct devmm_devid *devids)
{
    ...
    if ((svm_proc == NULL) || (devids == NULL)) {
        devmm_drv_err(
            "Invalid soma pool context. (svm_proc_is_null=%d; devids_is_null=%d)\n", (svm_proc == NULL),
            (devids == NULL));
        return -EINVAL;
    }
    ...
    if (devids->devid >= DEVMM_MAX_DEVICE_NUM) {
        devmm_drv_err(
            "Invalid soma pool device. (hostpid=%d; devid=%u)\n", svm_proc->process_id.hostpid, devids->devid);
        return -EINVAL;
    }
#endif
    return 0;
...
static int devmm_soma_create_pool_para_check(
    struct devmm_svm_process *svm_proc, struct devmm_soma_pool_create_para *para, struct devmm_devid *devids)
{
#ifndef EMU_ST
    ...
    if (para == NULL) {
        devmm_drv_err(
            "Create soma pool para is NULL. (hostpid=%d; devid=%u)\n", svm_proc->process_id.hostpid, devids->devid);
        return -EINVAL;
    }
#endif
...
#ifndef EMU_ST
        devmm_drv_err(
            "Invalid soma pool range. (hostpid=%d; devid=%u; pool_id=0x%llx; va=0x%llx; size=%llu)\n",
    ...
    if ((KA_DRIVER_IS_ALIGNED(para->va, DEVMM_HUGE_PAGE_SIZE) == false) ||
        ...
        devmm_drv_err(
            "Soma pool range is not aligned. (hostpid=%d; devid=%u; pool_id=0x%llx; va=0x%llx; size=%llu)\n",
            svm_proc->process_id.hostpid, devids->devid, para->pool_id, para->va, para->size);
        return -EINVAL;
    }
#endif
...
static int devmm_soma_destroy_pool_para_check(
    struct devmm_svm_process *svm_proc, struct devmm_soma_pool_destroy_para *para, struct devmm_devid *devids)
{
#ifndef EMU_ST
    int ret = devmm_soma_pool_check_proc_and_dev(svm_proc, devids);
    if (ret != 0) {
    ...
    if (para == NULL) {
        devmm_drv_err(
            "Destroy soma pool para is NULL. (hostpid=%d; devid=%u)\n", svm_proc->process_id.hostpid, devids->devid);
        return -EINVAL;
    }
#endif
    return 0;
...
int devmm_ioctl_soma_create_pool(struct devmm_svm_process *svm_proc, struct devmm_ioctl_arg *args)
{
    ...
    if (heap == NULL) {
        devmm_drv_err(
            "Mem pool get heap failed. (hostpid=%d; devid=%u; pool_id=0x%llx)\n", svm_proc->process_id.hostpid,
            devids->devid, para->pool_id);
        return -EINVAL;
    }
#endif
    ...
    ret = devmm_chan_msg_send(
        &msg, sizeof(struct devmm_chan_soma_pool_create), sizeof(struct devmm_chan_soma_pool_create));
    if (ret != 0) {
        devmm_drv_err(
            "Mem pool create msg send failed. (ret=%d; hostpid=%d; devid=%u; pool_id=0x%llx)\n", ret,
...
int devmm_ioctl_soma_destroy_pool(struct devmm_svm_process *svm_proc, struct devmm_ioctl_arg *args)
{
    ...
    msg.pool_id = (u64)para->pool_id;
    ret = devmm_chan_msg_send(
        &msg, sizeof(struct devmm_chan_soma_pool_destroy), sizeof(struct devmm_chan_soma_pool_destroy));
    if (ret != 0) {
        devmm_drv_err(
            "Mem pool destroy msg send failed. (ret=%d; hostpid=%d; devid=%u; pool_id=0x%llx)\n", ret,
            svm_proc->process_id.hostpid, devids->devid, para->pool_id);
        return ret;
    }
...
int devmm_ioctl_soma_trim_pool(struct devmm_svm_process *svm_proc, struct devmm_ioctl_arg *args)
{
    ...
    if ((KA_DRIVER_IS_ALIGNED(para->reserved_size, DEVMM_HUGE_PAGE_SIZE) == false) ||
        ...
        devmm_drv_err(
            "Soma pool trim size is not aligned. (hostpid=%d; devid=%u; pool_id=0x%llx; "
    ...
    if (ret != 0) {
        devmm_drv_err(
            "Mem pool trim msg send failed. (ret=%d; hostpid=%d; devid=%u; pool_id=0x%llx)\n", ret,
            svm_proc->process_id.hostpid, devids->devid, para->pool_id);
        return ret;
    }
