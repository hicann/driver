...
static void devmm_dma_desc_node_erase(struct devmm_dma_desc_node_rb_info *rb_info, struct devmm_dma_desc_node *node)
{
    (void)devmm_rb_erase(&rb_info->root, &node->task_node);
    rb_info->node_num--;
}

static int devmm_dma_desc_node_insert(struct devmm_dma_desc_node_rb_info *rb_info, struct devmm_dma_desc_node *node)
{
    int ret;
...
static struct devmm_dma_desc_node *devmm_dma_desc_node_erase_one_by_key(struct devmm_svm_process *svm_proc, u32 key)
{
    struct devmm_svm_proc_master *master_info = (struct devmm_svm_proc_master *)svm_proc->priv_data;
    struct devmm_dma_desc_node_rb_info *rb_info = devmm_dma_desc_get_rb_info(master_info, key);
