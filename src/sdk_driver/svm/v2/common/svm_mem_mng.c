...
static void devmm_print_svm_process_vma(ka_vm_area_struct_t *vma[], u32 vma_num)
{
    ...
    for (i = 0; i < vma_num; i++) {
        ...
        devmm_drv_err("Svm vma. (idx=%u; vm_start=0x%lx; vm_end=0x%lx; vm_pgoff=0x%lx; vm_flags=0x%lx)\n", i,
                      ka_mm_get_vm_start(vma[i]), ka_mm_get_vm_end(vma[i]), vma[i]->vm_pgoff,
                      ka_mm_get_vm_flags(vma[i]));
    }
}
...
static long _devmm_get_user_pages_remote(ka_task_struct_t *tsk, ka_mm_struct_t *mm, u64 va, int write, u32 num,
                                         ka_page_t **pages)
{
    long got_num;
    int locked;
