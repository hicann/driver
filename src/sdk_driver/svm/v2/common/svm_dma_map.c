...
static int _devmm_dma_map_pages(u32 devid, struct devmm_dma_map_page_info *pg_info, struct devmm_dma_blk *dma_blks,
                                u64 *blk_num)
{
    ka_device_t *dev = NULL;
    ...
    *blk_num = 0;
    ...
    for (i = 0, j = 0; i < pg_info->pg_num; j++, i += cont_num) {
        ...
        dma_blks[j].dma_addr = hal_kernel_devdrv_dma_map_page(dev, pg_info->pages[i], 0, dma_blks[j].size,
