    ...
    bool need_page_adjust)
{
    ...
    if (blk->attr.mem_map_route == MEM_MAP_EXBUS) {
        return devmm_remap_route_for_exbus(svm_proc, blk->src_devid, info, pages);
    } else {
...
static bool devmm_mem_map_need_free_ptes(struct devmm_phy_addr_blk *blk, u64 start, u64 size)
{
    if (blk->attr.is_giant_page) {
        return true;
    }

    /* Free pets are required in the following situations, for import type:
     * 1. Cross-OS memory import, 2M granularity memory mapping. will fail if va map again with 1GB
     * page table after unmapping.
     * 2. Same-OS memory import, 2M granularity memory mapping with. will fail if va map again with
     * 1GB page table after unmapping.
     */
    if ((blk->type == SVM_PYH_ADDR_BLK_IMPORT_TYPE) && (blk->attr.pg_type == MEM_HUGE_PAGE_TYPE) &&
        KA_DRIVER_IS_ALIGNED(start, DEVMM_GIANT_PAGE_SIZE) && KA_DRIVER_IS_ALIGNED(size, DEVMM_GIANT_PAGE_SIZE)) {
        return true;
    }
...
int devmm_mem_map(struct devmm_svm_process *svm_proc, struct devmm_vmma_info *info, bool need_page_adjust)
{
    ...
    if (devmm_mem_map_need_free_ptes(blk, info->va, info->size)) {
