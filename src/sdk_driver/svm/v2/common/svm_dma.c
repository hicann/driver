...
int devmm_dma_sync_link_copy_limit(struct devmm_copy_res *res, int instance)
{
    ...
    ret = devmm_bandwidth_limit_check((u32)res->dev_id, (u32)res->fid, (u32)res->dma_node->direction, (u32)res->cpy_len,
                                      res->dma_node_num);
    if (ret != 0) {
...
int devmm_dma_async_link_copy(struct devmm_copy_res *res, int instance, void *priv, dma_finish_notify call_back)
{
    ...
    devmm_drv_debug("Dma async link copy. (dev_id=%u; node_cnt=%u; src=%llx; dst=%llx; size=%x)", dev_id,
                    res->dma_node_num, res->dma_node[0].src_addr, res->dma_node[0].dst_addr, res->dma_node[0].size);
    ...
    ret = devmm_bandwidth_limit_check(dev_id, (u32)res->fid, (u32)res->dma_node->direction, (u32)res->cpy_len,
                                      res->dma_node_num);
    if (ret != 0) {
    ...
    if (sync_flag[dev_id] == 0) {
        ...
        /* d2d use source device dma. h2d&d2h use device dma */
        ret = hal_kernel_devdrv_dma_async_link_copy_plus(dev_id, DEVDRV_DMA_DATA_TRAFFIC, instance, res->dma_node,
                                                         res->dma_node_num, &para_info);
        /* dma queue is full, delay resubmit */
        if (ret == -ENOSPC) {
            sync_flag[dev_id] = 1;
            /* try sync link cpy */
