...
static int um_urma_seg_register_pre_handle(u32 udevid, int master_tgid, int slave_tgid, void *msg, u32 msg_len)
{
    ...
    if ((msg_head->va < va) || (msg_head->size > size) || (msg_head->va + msg_head->size > va + size)) {
        if (!va_is_in_sp_range(msg_head->va, msg_head->size) &&
            svm_mwl_check_mem(udevid, master_tgid, MWL_ID_NO_USE, msg_head->va, msg_head->size) != 0) {
            svm_err(
                "Invalid para. (udevid=%u; host_tgid=%d; va=0x%llx; size=0x%llx)\n", udevid, master_tgid, msg_head->va,
                msg_head->size);
            return -EINVAL;
        }
    }
    ...
    if (svm_urma_seg_flag_is_self_user(msg_head->flag)) {
        ...
        if (ret != 0) {
            svm_err(
                "Pma_ub register seg failed. (ret=%d; udevid=%u; tgid=%d; va=0x%llx; size=%llu)\n", ret, udevid,
                master_tgid, msg_head->va, msg_head->size);
            return ret;
        }
    }
...
static void um_urma_seg_register_pre_cancel_handle(u32 udevid, int master_tgid, int slave_tgid, void *msg, u32 msg_len)
{
    struct svm_urma_seg_msg_head *msg_head = (struct svm_urma_seg_msg_head *)msg;
    int ret;
    ...
    if (svm_urma_seg_flag_is_self_user(msg_head->flag)) {
        ...
        if (ret != 0) {
            svm_err(
                "Pma_ub unregister seg failed. (ret=%d; udevid=%u; tgid=%d; va=0x%llx; size=%llu)\n", ret, udevid,
                master_tgid, msg_head->va, msg_head->size);
        }
    }
}

static int um_urma_seg_register_post_handle(u32 udevid, int master_tgid, int slave_tgid, void *msg, u32 msg_len)
{
    struct svm_urma_seg_msg_head *msg_head = (struct svm_urma_seg_msg_head *)msg;
    int ret;
...
static int um_urma_seg_unregister_pre_handle(u32 udevid, int master_tgid, int slave_tgid, void *msg, u32 msg_len)
{
    struct svm_urma_seg_msg_head *msg_head = (struct svm_urma_seg_msg_head *)msg;
    u64 start, size;
    u32 token_id;
    ...
    if (svm_urma_seg_flag_is_self_user(msg_head->flag)) {
        ...
        if (ret != 0) {
            svm_err(
                "Pma_ub get seg info failed. (ret=%d; udevid=%u; tgid=%d; va=0x%llx)\n", ret, udevid, master_tgid,
                msg_head->va);
            return ret;
        }
        if (msg_head->va != start) {
            svm_err(
                "Pma_ub get seg info mismatch. (udevid=%u; tgid=%d; va=0x%llx; start=0x%llx)\n", udevid, master_tgid,
                msg_head->va, start);
            return -EINVAL;
        }
        ...
        if (ret != 0) {
            svm_err(
                "Pma_ub del seg failed. (ret=%d; udevid=%u; tgid=%d; va=0x%llx; size=%llu)\n", ret, udevid, master_tgid,
                start, size);
            return ret;
        }
...
void pma_ub_um_handle_init(void)
{
    svm_um_register_handle(
        SVM_URMA_SEG_REGISTER_EVENT, um_urma_seg_register_pre_handle, um_urma_seg_register_pre_cancel_handle,
        um_urma_seg_register_post_handle);
