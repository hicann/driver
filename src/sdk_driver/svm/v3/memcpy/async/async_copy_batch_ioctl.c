...
static int _async_copy_ioctl_submit_batch(struct async_copy_ctx *ctx, unsigned long __ka_user arg)
{
    ...
    if (ka_base_copy_to_user((void *)(uintptr_t)arg, (void *)&para, sizeof(struct svm_async_copy_submit_batch_para)) !=
        0) {
...
{
    svm_register_ioctl_cmd_handle(_KA_IOC_NR(SVM_ASYNC_COPY_SUBMIT_BATCH), async_copy_ioctl_submit_batch);
    return 0;
}
