...
struct tsagent_dev_base_info *tsagent_get_device_base_info(u32 dev_id)
{
    ...
    if ((g_dev_base_info[dev_id].chip_id != KA_U32_MAX) && (g_dev_base_info[dev_id].die_id != KA_U32_MAX) &&
        (g_dev_base_info[dev_id].addr_mode != KA_U32_MAX) && (g_dev_base_info[dev_id].soc_type != KA_U32_MAX)) {
    ...
    ts_agent_debug("get dev base info success, devid=%u, chip_id=%u, die_id=%u, addr_mode=%d, soc_type=%u.", dev_id,
                   chip_id, die_id, addr_mode, soc_type);
...
void tsagent_sq_info_set(u32 devid, u32 stream_id, u16 sqid, bool swsq_flag)
{
    if ((devid >= TSAGENT_MAX_DEV_ID) || (sqid >= TS_AGENT_MAX_SQ_NUM)) {
        ts_agent_err("devid or sqid is invalid. "
                     "(devid=%u, valid_devid_range=[0, %u), sqid=%u, valid_sqid_range=[0, %u))",
                     devid, TSAGENT_MAX_DEV_ID, sqid, TS_AGENT_MAX_SQ_NUM);
        return;
    }

    g_sq_base_info[devid][sqid].swsq_flag = swsq_flag;
    ts_agent_debug("devid=%u, stream_id=%u, sqid=%u, swsq_flag=%d.", devid, stream_id, sqid, swsq_flag);
}
...
{
    size_t i;
    ts_agent_err("dump error sqe, dev_id=%u, stream_id=%u, task_id=%u, sqe_type=%u", devid, sqe->stream_id,
                 sqe->task_id, sqe->type);
    ...
    for (i = 0U; i < (sizeof(ts_stars_sqe_t) / sizeof(u32)); i += 8U) {
        ts_agent_err("word[%zu-%zu]: 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x.", i, i + 7U,
                     ((const u32 *)sqe)[i + 0U], ((const u32 *)sqe)[i + 1U], ((const u32 *)sqe)[i + 2U],
                     ((const u32 *)sqe)[i + 3U], ((const u32 *)sqe)[i + 4U], ((const u32 *)sqe)[i + 5U],
                     ((const u32 *)sqe)[i + 6U], ((const u32 *)sqe)[i + 7U]);
    }
}
...
static int sqe_proc_dvpp(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (g_dvpp_ops.dvpp_sqe_update == NULL) {
        ts_agent_warn("dvpp ops is not registered!");
        return EOK;
    }

    ret = g_dvpp_ops.dvpp_sqe_update(devid, tsid, pid, sqe);
    if (ret != EOK) {
        ts_agent_err("dvpp update is error, ret=%d, devid=%u, stream_id=%u, task_id=%u, pid=%d, sqe_type=%u", ret,
                     devid, sqe->stream_id, sqe->task_id, pid, sqe->type);
    }
    return ret;
}
...
static int sqe_proc_cdqm(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (is_match == false) {
        ts_agent_err("cdq sqe check failed, ret=%d, devid=%u, stream_id=%u, task_id=%u, pid=%d, cdq_id=%u", is_match,
                     devid, sqe->stream_id, sqe->task_id, pid, cdq_sqe->cdq_id);
        return -EINVAL;
    }
...
static int pcie_dma_desc_check(u32 devid, ts_stars_pciedma_sqe_t *sqe)
{
    ...
    if (ret != 0) {
        ts_agent_err("pciedma desc check failed, ret=%d, devid=%u, stream_id=%u, task_id=%u, src=%#llx, dst=%#llx, "
                     "len=%#llx, dma sq addr=%#llx, dma sq tail=%llu.",
                     ret, devid, sqe->header.rt_stream_id, sqe->header.task_id, sqe->src, sqe->dst, sqe->length,
                     check_dma.sq_dma_addr, check_dma.sq_size);
        return ret;
    }
#endif
    return EOK;
...
static int sqe_proc_vm_pciedma_for_update_sqe(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_pciedma_sqe_t *pcie_sqe)
{
    ...
    if (ret != 0) {
        ts_agent_err("get ssid failed, ret=%d, devid=%u, pid=%d, tsid=%u, sq_id=%u", ret, devid, pid, tsid, sqid);
        return -EINVAL;
    }
    ...
    if (ret != 0) {
        ts_agent_err("trans v2p failed, ret=%d, devid=%u, tsid=%d, sq_id=%u", ret, devid, tsid, sqid);
        return -EINVAL;
    }
                                            ...
                                            ts_stars_pciedma_sqe_t *pcie_sqe)
{
    ...
    ret = memcpy_s((void *)pcie_sqe, sizeof(ts_stars_pciedma_sqe_t), (void *)&sdma_sqe,
                   sizeof(ts_stars_memcpy_async_sqe_t));
    if (ret != EOK) {
...
#if (defined CFG_SOC_PLATFORM_CLOUD_V2) && (!defined CFG_HOST_VIRTUAL_MACHINES)
static void sqe_proc_pm_pciedma_fill_addr_info(int pid, u32 sqid, u32 passid, ts_stars_pciedma_sqe_t *pcie_sqe,
...
static int sqe_proc_pm_pciedma_for_update_sqe(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_pciedma_sqe_t *pcie_sqe)
{
    ...
    if (ret != 0) {
        ts_agent_err("pciedma addr create failed, ret=%d, devid=%u, pid=%d, sq_id=%u, passid=%u", ret, devid, pid, sqid,
                     passid);
        return ret;
    }
    ...
    ts_agent_info("pciedma addr create success, devid=%u, sq_id=%u, pos=%u, pid=%d, src=%#llx, cnt=%llu, "
                  "dma sq addr=%#lx, dma sq tail=%u, type=%u, ssid=%u",
                  devid, sqid, pos, pid, pcie_sqe->src, pcie_sqe->length, (uintptr_t)dma.pciedma_desc.sq_addr,
                  dma.pciedma_desc.sq_tail, pcie_sqe->header.type, passid);
#endif
    return EOK;
}
#endif
...
static int sqe_proc_pciedma_for_update_sqe(u32 devid, u32 tsid, int pid, ts_stars_sqe_t *sqe)
{
    ...
    if ((pcie_sqe->is_sqe_update == 1U) && ((pcie_sqe->length == 0ULL) || (pcie_sqe->length > TS_AGENT_SQE_SIZE) ||
                                            ((pcie_sqe->length + pcie_sqe->offset) > TS_AGENT_SQE_SIZE))) {
        ts_agent_err("update cp size is invalid, invalid_size=%llu, invalid_offset=%u", pcie_sqe->length,
                     pcie_sqe->offset);
        return -EINVAL;
    }

    if ((pcie_sqe->is_dsa_update == 1U) && (pcie_sqe->length != dsa_cp_size)) {
        ts_agent_err("dsa cp size is invalid, invalid_size=%llu, correct_size=%llu", pcie_sqe->length, dsa_cp_size);
        return -EINVAL;
    }
    ...
    if (is_match == false) {
        ts_agent_err("sq_id check failed, devid=%u, pid=%d, sq_id=%u", devid, pid, sqid);
        return -EINVAL;
    }
#endif
    ...
    if (ret != 0) {
        ts_agent_err("dsa update failed, ret=%d, sqe_type=%u, devid=%u, sq_id=%u, pid=%d.", ret, sqe->type, devid, sqid,
                     pid);
...
static int sqe_proc_pciedma(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if ((pcie_sqe->is_dsa_update == 1U) || (pcie_sqe->is_sqe_update == 1U)) {
        if (sqid == KA_U32_MAX) {
            ts_agent_err(
                "not support sqe_proc_pciedma for decoupled stream_id and sq_id scenario: sqid=%u, stream_id=%u", sqid,
                sqe->stream_id);
            return EOK;
        }
    ...
    if (pcie_sqe->is_converted == 0U) {
        ...
        if (ret != 0) {
            ts_agent_err("pciedma addr convert failed, ret=%d, devid=%u, stream_id=%u, task_id=%u, pid=%d, "
                         "src=%#llx, dst=%#llx, len=%#llx, is_not_delete=%u.",
        ...
        if (ret != 0) {
            if (ret != -ESRCH) {
                ts_agent_err("pciedma addr convert failed, ret=%d, devid=%u, stream_id=%u, task_id=%u, pid=%d, "
                             "src=%#llx, dst=%#llx, len=%#llx.",
                             ret, devid, sqe->stream_id, sqe->task_id, pid, pcie_sqe->src, pcie_sqe->dst,
                             pcie_sqe->length);
            }
            return ret;
        }
        ...
        ts_agent_debug("devid=%u, stream_id=%u, task_id=%u, pid=%d, src=%#llx, dst=%#llx, len=%#llx, dma sq addr=%#lx, "
                       "dma sq tail=%u.",
                       devid, sqe->stream_id, sqe->task_id, pid, pcie_sqe->src, pcie_sqe->dst, pcie_sqe->length,
...
static int sqe_proc_sdma(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (sdma_sqe->ptr_mode == 0U) {
        ...
        ts_agent_err("sdma sqe check failed, devid=%u, stream_id=%u, task_id=%u, pid=%d, sssv=%u, dssv=%u", devid,
                     sqe->stream_id, sqe->task_id, pid, sdma_sqe->sssv, sdma_sqe->dssv);
        return -EINVAL;
    } else {
        ts_stars_memcpy_ptr_async_sqe_t *sdma_ptr_sqe = (ts_stars_memcpy_ptr_async_sqe_t *)sqe;
        if (sdma_ptr_sqe->va == 0ULL) {
            ts_agent_err("sdma sqe check failed, devid=%u, stream_id=%u, task_id=%u, pid=%d, ptr_mode=%u, va=%u", devid,
                         sqe->stream_id, sqe->task_id, pid, sdma_ptr_sqe->ptr_mode, sdma_ptr_sqe->va);
            return -EINVAL;
        }
    }
...
static int sqe_proc_write_value_check_event_addr(u32 devid, u32 tsid, int pid, uint64_t reg_addr)
{
    ...
    if (is_match == false) {
        ts_agent_err("write value event check failed, ret=%d, devid=%u, pid=%d, event_id=%u", is_match, devid, pid,
                     event_id);
        return -EINVAL;
    }
#endif
...
#endif
    if (is_match == false) {
        ts_agent_err("notifyid check failed,pid=%d,notify_id=%#llx,tsid=%u,addr=%#llx,devid=%u", pid, notify_id, tsid,
                     reg_addr, devid);
        return -EINVAL;
    }
#endif
...
static int sqe_proc_write_value_check_rdma_addr(u32 devid, u32 tsid, int pid, uint64_t reg_addr)
{
    ...
    if ((dev_base_info->soc_type == SOC_TYPE_CLOUD_V2) || (dev_base_info->soc_type == SOC_TYPE_CLOUD_V3)) {
        ...
        dbAddr = TS_ROCEE_BASE_ADDR + TS_ROCEE_VF_DB_CFG0_REG + chipOffset * dev_base_info->chip_id +
                 dieOffset * dev_base_info->die_id + chipBaseAddr;
        if (reg_addr != dbAddr) {
            ts_agent_err("rdma addr base check failed, devid=%u, pid=%d, addr=%#llx, expected dbAddr=%#llx, "
        ...
        ts_agent_err("rdma addr base check failed, devid=%u, pid=%d, addr=%#llx, "
                     "chip_id=%u, die_id=%u, addr_mode=%u, soc_type=%u",
                     devid, pid, reg_addr, dev_base_info->chip_id, dev_base_info->die_id, dev_base_info->addr_mode,
                     dev_base_info->soc_type);
        return -EINVAL;
    }
...
static int sqe_proc_write_value(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (!KA_MM_IS_ALIGNED((wv_sqe->write_addr_low), (1U << wv_sqe->awsize))) {
        ts_agent_err("addr aligned check failed, awsize=%u, write_addr_low=%#x", wv_sqe->awsize,
                     wv_sqe->write_addr_low);
        goto ERR_PROC;
    }
...
ERR_PROC:
    ts_agent_err("write value check failed, devid=%u, stream_id=%u, task_id=%u, pid=%d, addr_h=%#x, addr_l=%#x, "
                 "sub_type=%u",
                 devid, sqe->stream_id, sqe->task_id, pid, wv_sqe->write_addr_high, wv_sqe->write_addr_low,
                 wv_sqe->sub_type);
    return -EINVAL;
}
...
static int sqe_update_for_all_ffts_plus(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (is_match == false) {
        ts_agent_err("sqe check failed, devid=%u, tsid=%u, stream_id=%u, task_id=%u, pid=%d, sqid=%u", devid, tsid,
                     tmp_stream_id, ffts_sqe->header.task_id, pid, sqid);
        return -EINVAL;
    }
    ...
    ts_agent_debug("update fftsplus sqe for all, devid=%u, tsid=%u, pid=%d, sqid=%u, "
                   ...
                   devid, tsid, pid, sqid, real_stream_id, real_task_id, ffts_sqe->header.rt_stream_id,
                   ffts_sqe->header.task_id);
#endif
    return EOK;
}
...
static int sqe_proc_ffts(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    if (!KA_MM_IS_ALIGNED((ffts_sqe->context_address_base_l), 128U)) { // 128 aligned
        ts_agent_err("addr aligned check failed, devid=%u, stream_id=%u, task_id=%u, pid=%d, address_base_l=%#x", devid,
                     sqe->stream_id, sqe->task_id, pid, ffts_sqe->context_address_base_l);
        return -EINVAL;
    }
#ifndef TS_AGENT_UT
    if (uda_is_phy_dev(devid) == false) {
        ts_agent_debug("sqe pid update success, devid=%u, stream_id=%u, task_id=%u, pid=%d.", devid, sqe->stream_id,
                       sqe->task_id, pid);
...
static int check_mnt_ns(int pid, ts_stars_sqe_t *sqe)
{
    ...
    if (tp_sqe->dest_pid_flag != 0U) {
        ...
        if (tp_sqe->usr_data_len < sizeof(dest_pid)) {
            ts_agent_err("topic_sched sqe usr_data_len should be >= 4 Bytes when dest_pid_flag was set, "
                         "usr_data_len=%u.",
                         tp_sqe->usr_data_len);
            return -EINVAL;
        }
        ...
        if (src_mnt_ns != dest_mnt_ns) {
            ts_agent_err("src_pid and dest_pid are not in the same mnt namespace, src_pid=%d, dest_pid=%d.", pid,
                         (int)(dest_pid));
            return -EINVAL;
        }
    }
...
static int sqe_proc_topic(u32 devid, u32 tsid, int pid, u32 sqid, ts_stars_sqe_t *sqe)
{
    ...
    topic_sqe->p_l2ctrl_low = (topic_sqe->kernel_type == TS_CPU_KERNEL_TYPE_CUSTOM_KFC) ? (u32)pid :
                                                                                          topic_sqe->p_l2ctrl_low;
    if (uda_is_phy_dev(devid) == false) {
...
{
    if ((sqe->type != (uint8_t)TS_STARS_SQE_TYPE_NOTIFY_WAIT) && (sqe->type != (uint8_t)TS_STARS_SQE_TYPE_EVENT_WAIT) &&
        (sqe->type != (uint8_t)TS_STARS_SQE_TYPE_AICPU) && (sqe->type != (uint8_t)TS_STARS_SQE_TYPE_FFTS)) {
        ts_agent_err("kernel credit can't be 0xFF, sqe_type=%u, devid=%u, stream_id=%u, task_id=%u, pid=%d.", sqe->type,
...
int tsagent_sqe_update(u32 devid, u32 tsid, struct trs_sqe_update_info *update_info)
{
    ...
    ts_agent_debug("sqe update success, sqe_type=%u, devid=%u, stream_id=%u, task_id=%u, pid=%d.", sqe->type, devid,
                   sqe->stream_id, sqe->task_id, update_info->pid);
    return EOK;
}
...
{
    u64 ts_sram_rsv;
    ts_agent_info("tsagent device init, devid=%u, tsid=%u, agent_para_size=0x%llx.", devid, tsid,
                  (u64)(para->rsv_size));
    if (devid >= TSAGENT_MAX_DEV_ID) {
...
int tsagent_cqe_update(u32 devid, u32 tsid, int pid, u32 cqid, void *cqe)
{
    ...
    ts_agent_debug("stars cqe pid=%d, old stream_id=%hu, task_id=%hu, new stream_id=%hu, task_id=%hu", pid, stream_id,
                   task_id, stars_cqe->stream_id, stars_cqe->task_id);
#endif
#endif
    ...
    ts_agent_debug("cqe debug, devid=%u, pid=%d, stream_id=%u, task_id=%u, sqid=%u, sqetype=%u, sq_head=%u, "
                   "place_hold=%d, evt=%d, error_bit=%d.",
                   devid, pid, stars_cqe->stream_id, stars_cqe->task_id, stars_cqe->sq_id, stars_cqe->sqe_type,
                   stars_cqe->sq_head, stars_cqe->place_hold, stars_cqe->evt, stars_cqe->error_bit);
#endif
...
static void dma_des_destroy_by_stream_id(u32 devid, int pid, u32 stream_id)
{
    if (stream_id != KA_U32_MAX) {
        ...
        if (dma_cnt_max_last[devid] < dma_cnt_max[devid]) {
            ...
            ts_agent_debug("peak value of dma desc is: devid=%u, dma_cnt=%ld.", devid, dma_cnt_max_last[devid]);
        }
    }
...
#endif
        ts_agent_debug("try to destroy stream all dma desc, devid=%u, pid=%d, stream_id=%u, sqid=%u", devid, pid,
                       stream_id, mb->u.cmd_sqcq_info.sq_idx);
    }
