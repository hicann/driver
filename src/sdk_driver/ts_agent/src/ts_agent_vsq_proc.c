...
STATIC int convert_event_record_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ...
        ts_agent_debug("convert v_event_id=%u to event_id=%u. v_stream_id=%u.", v_event_id, event_record_task->eventID,
                       v_stream_id);
    }
    return ret;
}
...
STATIC int convert_stream_wait_event_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ...
        ts_agent_debug("convert v_event_id=%u to event_id=%u. v_stream_id=%u.", v_event_id,
                       stream_wait_event_task->eventID, v_stream_id);
    }
    return ret;
}
...
STATIC int convert_maintenance_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if ((goal == MAINTENANCE_GOAL_STREAM) || (goal == MAINTENANCE_GOAL_STREAM_TASK_RECYCLE)) {
        ...
        if (ret != 0) {
            ...
            ts_agent_debug("convert v_stream_id=%u to event_id=%u. v_stream_id=%u.", target_id,
                           maintenance_task->targetID, v_stream_id);
        }
        ...
        if (ret != 0) {
            ...
            maintenance_task->v_target_id = target_id;
            ts_agent_debug("convert v_event_id=%u to event_id=%u. v_stream_id=%u.", target_id,
                           maintenance_task->targetID, v_stream_id);
        }
    } else {
        ret = 0;
...
static int convert_create_stream_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    ts_agent_debug("convert create stream task end. vf_id=%u, v_stream_id=%u.", vsq_base_info->vf_id, task->streamID);
    return 0;
}
...
STATIC int convert_notify_wait_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ts_agent_err("convert notify id failed. v_stream_id=%u, v_notify_id=%u, ret=%d.", v_stream_id, v_notify_id,
                     ret);
        notify_wait_task->notify_id = KA_U16_MAX;
    } else {
        ts_agent_debug("convert v_notify_id=%u to notify_id=%u. v_stream_id=%u.", v_notify_id,
                       notify_wait_task->notify_id, v_stream_id);
    }
    return ret;
}
...
STATIC int convert_model_maintaince_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (convert_model_ret != 0) {
        ts_agent_err("convert model id failed. v_stream_id=%u, v_model_id=%u, ret=%d.", v_stream_id, v_model_id,
                     convert_model_ret);
        model_maintaince_task->model_id = KA_U16_MAX;
        // convert failed need convert sub stream_id also.
    } else {
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id,
                       model_maintaince_task->model_id, v_stream_id);
    }
    ...
    if (convert_stream_ret != 0) {
        ts_agent_err("convert stream id failed. v_stream_id=%u, v_sub_stream_id=%u, ret=%d.", v_stream_id,
                     v_sub_stream_id, convert_stream_ret);
        model_maintaince_task->stream_id = KA_U16_MAX;
    } else {
        ts_agent_debug("convert v_sub_stream_id=%u to sub_stream_id=%u. v_stream_id=%u.", v_sub_stream_id,
                       model_maintaince_task->stream_id, v_stream_id);
    }
...
STATIC int convert_model_execute_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (convert_model_ret != 0) {
        ...
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id,
                       model_execute_task->model_id, v_stream_id);
    }
...
STATIC int convert_stream_switch_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ts_agent_err("convert true stream id failed. v_stream_id=%u, v_true_stream_id=%u, ret=%d.", v_stream_id,
                     v_true_stream_id, ret);
        stream_switch_task->trueStreamId = KA_U16_MAX;
    } else {
        ts_agent_debug("convert v_true_stream_id=%u to true_stream_id=%u. v_stream_id=%u.", v_true_stream_id,
                       stream_switch_task->trueStreamId, v_stream_id);
    }
    return 0;
}
...
STATIC int convert_stream_active_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ts_agent_err("convert active stream id failed. v_stream_id=%u, v_active_stream_id=%u, ret=%d.", v_stream_id,
                     v_active_stream_id, ret);
        stream_active_task->activeStreamId = KA_U16_MAX;
    } else {
        ts_agent_debug("convert v_active_stream_id=%u to active_stream_id=%u. v_stream_id=%u.", v_active_stream_id,
                       stream_active_task->activeStreamId, v_stream_id);
    }
    return 0;
}
...
STATIC int convert_event_reset_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ts_agent_err("convert event id failed. v_stream_id=%u, v_event_id=%u, ret=%d.", v_stream_id, v_event_id, ret);
        event_reset_task->eventID = KA_U16_MAX;
    } else {
        ts_agent_debug("convert v_event_id=%u to event_id=%u. v_stream_id=%u.", v_event_id, event_reset_task->eventID,
                       v_stream_id);
    }
    return ret;
}
...
STATIC int convert_model_end_graph_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ts_agent_err("convert model id failed. v_stream_id=%u, v_model_id=%u, ret=%d.", v_stream_id, v_model_id, ret);
        end_graph_task->model_id = KA_U16_MAX;
    } else {
        end_graph_task->model_id = model_id;
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id, end_graph_task->model_id,
                       v_stream_id);
    }
    return ret;
}
...
STATIC int convert_model_exit_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (convert_model_ret != 0) {
        ...
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id, model_exit_task->model_id,
                       v_stream_id);
    }
    ...
    if (convert_stream_ret != 0) {
        ...
        ts_agent_debug("convert v_sub_stream_id=%u to sub_stream_id=%u. v_stream_id=%u.", v_sub_stream_id,
                       model_exit_task->stream_id, v_stream_id);
    }
    return convert_model_ret == 0 ? convert_stream_ret : convert_model_ret;
}
...
static int convert_aicpu_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ...
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id, aicpu_task->model_id,
                       v_stream_id);
    }
    return ret;
}
...
static int convert_stream_label_goto_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (ret != 0) {
        ...
        ts_agent_debug("convert v_model_id=%u to model_id=%u. v_stream_id=%u.", v_model_id,
                       stream_label_goto_task->model_id, v_stream_id);
    }
    return ret;
}
...
{
    ts_agent_err("task is not supported. vsq_id=%u, v_stream_id=%u, task_id=%u, task_type=%u.", vsq_base_info->vsq_id,
                 task->streamID, task->taskID, task->type);
    return -EPERM;
}
...
static int convert_task(const vsq_base_info_t *vsq_base_info, ts_task_t *task)
{
    ...
    if (task->type >= TS_TASK_TYPE_RESERVED) {
        ts_agent_err("task type is invalid. task_type=%u, task_id=%u.", task->type, task->taskID);
        return -EINVAL;
    }
    custom_fn = g_task_convert_fn[task->type];
    // no custom convert function is allowed.
    if (custom_fn == NULL) {
        ts_agent_err("task does not config convert function. task_type=%u, task_id=%u.", task->type, task->taskID);
        custom_convert_ret = -EINVAL;
    } else {
        custom_convert_ret = custom_fn(vsq_base_info, task);
        if (custom_convert_ret != 0) {
            ts_agent_err("custom convert task failed. ret=%d, task_type=%u, task_id=%u.", custom_convert_ret,
    ...
    if (ret != 0) {
        ts_agent_err("convert task basic failed. ret=%d, task_type=%u, task_id=%u.", ret, task->type, task->taskID);
        task->task_info_flag |= TS_TASK_INVALID_FLAG; /* set invalid flag */
        return ret;
    }
    ...
    switch (vsq_base_info->vsq_type) {
        ...
        case CALLBACK_VSQCQ_TYPE: {
            ...
            if (ret != 0) {
                ...
                ts_agent_debug("callback sq stream id needs to be converted now. dev_id=%u, vf_id=%u, ts_id=%u, "
                               ...
                               callback_msg->stream_id, callback_msg->task_id, v_stream_id);
            }
            break;
        }
        default: {
...
static int proc_vsq_by_range(const vsq_base_info_t *vsq_base, u32 head, u32 tail)
{
    ...
    ts_agent_debug("proc vsq, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, head=%u, tail=%u.", vsq_base->dev_id,
                   vsq_base->vf_id, vsq_base->ts_id, vsq_base->vsq_id, head, tail);
    ...
    while (curr_head != tail) {
        ...
        if (cp_ret != EOK) {
            ts_agent_err("memcpy task from vsq failed. cp_ret=%d, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, "
                         "vsq_slot_addr=%p, vsq_slot_size=%u, head=%u.",
                         cp_ret, vsq_base->dev_id, vsq_base->vf_id, vsq_base->ts_id, sq_slot.vsq_id, vsq_base_addr,
                         vsq_slot_size, head);
            ret = -EINVAL;
            break;
        }
        ...
        if (ret != 0) {
            ts_agent_err("proc_task failed. ret=%d, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, vsq_type=%d.", ret,
                         vsq_base->dev_id, vsq_base->vf_id, vsq_base->ts_id, vsq_base->vsq_id, vsq_base->vsq_type);
            // can not break, as error task is need send to ts also,
            // if not send, may lost cq report and can't update vsq_head.
        }
        ret = hal_kernel_hvtsdrv_sq_write(vsq_base->dev_id, vsq_base->vf_id, vsq_base->ts_id, &sq_slot);
        if (ret != 0) {
            ts_agent_err("Driver sq write failed. ret=%d, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, vsq_slot_addr=%p,"
                         " vsq_slot_size=%u.",
                         ret, vsq_base->dev_id, vsq_base->vf_id, vsq_base->ts_id, sq_slot.vsq_id, sq_slot.vsq_slot_addr,
                         sq_slot.vsq_slot_size);
            break;
        }
        ++deal_count;
        ++curr_head;
...
void proc_vsq(vsq_base_info_t *vsq_base_info)
{
    ...
    int retry_times = 1;
    ts_agent_debug("proc vsq begin, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u.", vsq_base_info->dev_id,
    ...
    ts_agent_debug("proc vsq end, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u.", vsq_base_info->dev_id,
                   vsq_base_info->vf_id, vsq_base_info->ts_id, vsq_base_info->vsq_id);
}
