...
#define VF_WORK(_dev_id, _vf_id, _ts_id) g_all_vf_worker[_dev_id][0][_ts_id]
#else
#define VF_WORK(_dev_id, _vf_id, _ts_id) g_all_vf_worker[_dev_id][_vf_id][_ts_id]
#endif
...
static int create_vsq_work_ctx(vsq_work_ctx_t *vsq_work_ctx)
{
    ...
    ret = sprintf_s(work_queue_name, TS_AGENT_MAX_WQ_NAME_LEN, "tsa_%u_%u_%u_%u", vsq_base_info->dev_id,
                    vsq_base_info->vf_id, vsq_base_info->ts_id, vsq_base_info->vsq_id);
    if (ret == -1) {
        ts_agent_err("format work queue name failed, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u.", vsq_base_info->dev_id,
                     vsq_base_info->vf_id, vsq_base_info->ts_id, vsq_base_info->vsq_id);
        return EFAULT;
    }
...
#endif
    ts_agent_debug("create vsq work ctx success, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u", vsq_base_info->dev_id,
                   vsq_base_info->vf_id, vsq_base_info->ts_id, vsq_base_info->vsq_id);
    return 0;
}
...
int create_vf_worker(u32 dev_id, u32 vf_id, u32 ts_id, u32 vsq_num)
{
    ...
    vf_work_ctx = &VF_WORK(dev_id, vf_id, ts_id);
    if (vf_work_ctx->vsq_num > 0 || vf_work_ctx->vsq_work_ctx_list != NULL) {
        ts_agent_err("vf work ctx repeat create, dev_id=%u, vf_id=%u, ts_id=%u, vsq_num=%u, last vsq_num=%u", dev_id,
                     vf_id, ts_id, vsq_num, vf_work_ctx->vsq_num);
        return -EINVAL;
    }
    ...
    if (vf_work_ctx->vsq_work_ctx_list == NULL) {
        ts_agent_err("ka_mm_kzalloc failed, dev_id=%u, vf_id=%u, vsq_num=%u, size=%u", dev_id, vf_id,
                     vf_work_ctx->vsq_num, size);
        vf_work_ctx->vsq_num = 0;
        return -ENOMEM;
    }
    for (vsq_id = 0; vsq_id < vsq_num; ++vsq_id) {
        ...
        if (ret != 0) {
            ts_agent_err("fill vsq info failed, dev_id=%u, vf_id=%u, vsq_num=%u, vsq_id=%u", dev_id, vf_id,
                         vf_work_ctx->vsq_num, vsq_id);
            // rollback
            destroy_vf_work_ctx(vf_work_ctx);
            return ret;
        }
        ...
        if (ret != 0) {
            ts_agent_err("create vsq work ctx failed, dev_id=%u, vf_id=%u, vsq_num=%u, vsq_id=%u", dev_id, vf_id,
                         vf_work_ctx->vsq_num, vsq_id);
            // rollback
            destroy_vf_work_ctx(vf_work_ctx);
            return ret;
    ...
    ts_agent_info("create vf worker end, dev_id=%u, vf_id=%u, ts_id=%u, vsq_num=%u", dev_id, vf_id, ts_id, vsq_num);
    return 0;
}
...
int schedule_vsq_work(const struct tsdrv_id_inst *const id_inst, u32 vsq_id, enum vsqcq_type vsq_type, u32 cmd_num)
{
    ...
    if (wq_sq_id >= vf_work_ctx->vsq_num) {
        ts_agent_err("wq_sq_id or vf_id is invalid, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, vsq_num=%u", dev_id,
                     vf_id, ts_id, vsq_id, vf_work_ctx->vsq_num);
        return -ERANGE;
    }
    if (vf_work_ctx->vsq_work_ctx_list == NULL) {
        ts_agent_err("vsq_work_ctx is null, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, vsq_num=%u", dev_id, vf_id, ts_id,
                     vsq_id, vf_work_ctx->vsq_num);
        return -EINVAL;
    }
    ...
    if (ret != 0) {
        ts_agent_err("vsq top proc failed, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u, cmd_num=%u", dev_id, vf_id, ts_id,
                     vsq_id, cmd_num);
    }

    if (ka_task_queue_work(vsq_work_ctx->wq, &vsq_work_ctx->proc_work)) {
        ts_agent_debug("schedule work success, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u", dev_id, vf_id, ts_id, vsq_id);
    } else {
        ts_agent_debug("task is in schedule already, dev_id=%u, vf_id=%u, ts_id=%u, vsq_id=%u", dev_id, vf_id, ts_id,
                       vsq_id);
    }
    return ret;
}
