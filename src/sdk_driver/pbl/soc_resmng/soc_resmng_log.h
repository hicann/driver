...
#define soc_err(fmt, ...)                                                                                  \
    do {                                                                                                   \
        drv_err(module_soc_resmng, "<%s:%d> " fmt, ka_task_get_current_comm(), ka_task_get_current_tgid(), \
                ##__VA_ARGS__);                                                                            \
    } while (0)
#else
#define soc_err(fmt, ...)                                                                                   \
...
#define soc_debug(fmt, ...)                                                                                 \
    drv_pr_debug(module_soc_resmng, "<%s:%d> " fmt, ka_task_get_current_comm(), ka_task_get_current_tgid(), \
                 ##__VA_ARGS__)
