...
int devdrv_send_vdm_msg(u32 dev_id)
{
    ...
    if (dev_ops == NULL) {
        devdrv_err("Get msg chan fail. (dev_id=%u)\n", dev_id);
        return -EOPNOTSUPP;
    }
    (void)uda_udevid_to_add_id(dev_id, &index_id);
    if (dev_ops->ops.send_vdm_msg == NULL) {
        devdrv_err("ops.send_vdm_msg is null.\n");
        devdrv_sub_ops_ref(dev_ops);
        return -EOPNOTSUPP;
    }
    ret = dev_ops->ops.send_vdm_msg(index_id);
    devdrv_sub_ops_ref(dev_ops);
    return ret;
}
KA_EXPORT_SYMBOL(devdrv_send_vdm_msg);