...
int dcmi_clear_config_para_check(int card_id, const char *config_name)
{
    if (!dcmi_is_in_phy_machine_root()) {
        gplog(LOG_OP, "Operation not permitted, only root user on physical machine can call this api.");
        return DCMI_ERR_CODE_OPER_NOT_PERMITTED;
...
int dcmi_clear_device_user_config(int card_id, int device_id, const char *config_name)
{
    ...
    err = dcmi_clear_config_para_check(card_id, config_name);
    if (err != DCMI_OK) {
        gplog(LOG_ERR, "dcmi_clear_config_para_check failed. err is %d.");
        return err;
    }
...
int dcmi_check_multi_die_policy_config_recover_mode_is_permitted(const char *operate_mode)
{
    ...
    if (dcmi_board_chip_type_is_ascend_910_93()) {
        ...
        gplog(LOG_OP, "This device does not support %s multi-die-policy config recover mode.", operate_mode);
        return DCMI_ERR_CODE_NOT_SUPPORT;
    }
    return DCMI_OK;
}

int dcmi_set_multi_die_policy_config_recover_mode(unsigned int enable_flag)
{
    int ret;

    ret = dcmi_check_multi_die_policy_config_recover_mode_is_permitted("set");
    if (ret != DCMI_OK) {
        return ret;
    }
    ...
    if ((enable_flag != DCMI_CFG_RECOVER_ENABLE) && (enable_flag != DCMI_CFG_RECOVER_DISABLE)) {
        gplog(LOG_ERR,
            "dcmi_set_multi_die_policy_config_recover_mode: parameter enable_flag [%u] is invalid.", enable_flag);
        return DCMI_ERR_CODE_INVALID_PARAMETER;
    }

    ret = dcmi_cfg_set_multi_die_policy_config_recover_mode(enable_flag);
    if (ret != DCMI_OK) {
        gplog(LOG_ERR, "set multi-die-policy config recover enable_flag %u failed. ret=%d", enable_flag, ret);
        return ret;
    }

    gplog(LOG_OP, "dcmi set multi-die-policy config recover enable_flag %s success",
...
static int dcmi_set_config_para_check(const char *config_name, unsigned int buf_size, char *buf)
{
    int err;
...
int dcmi_set_device_user_config(int card_id, int device_id, const char *config_name, unsigned int buf_size, char *buf)
{
    ...
    err = dcmi_set_config_para_check(config_name, buf_size, buf);
    if (err != DCMI_OK) {
        return err;
    }
