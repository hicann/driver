...
static int trs_get_segment(
    struct trs_urma_ctx *urma_ctx, uint64_t va, uint64_t size, struct trs_async_dma_wqe_info *wqe_info, urma_seg_t *seg,
    urma_token_t *token)
{
    int ret = 0;
...
static inline void trs_get_async_send_recv_addr(
    struct trs_async_devid *async_devid, uint64_t src, uint64_t dst, uint64_t *send_addr, uint64_t *recv_addr)
{
...
static struct trs_async_ctx *trs_get_async_ctx(
    struct trs_urma_ctx *urma_ctx, uint32_t dir, enum trs_async_dma_type async_dma_type)
{
...
static void trs_get_async_input_src_dst_len_array(
    struct trs_async_dma_input_para *in, unsigned long long **src_array, unsigned long long **dst_array,
    unsigned long long **src_len_array, unsigned long long **dst_len_array)
{
...
static drvError_t trs_check_d2d_async_devid(uint32_t dev_id, uint32_t src_dev_id, uint32_t dst_dev_id)
{
    ...
    if ((src_dev_id == host_dev_id) || (dst_dev_id == host_dev_id)) {
        trs_err(
            "Invalid dev_id. (host_dev_id=%u; src_dev_id=%u; dst_dev_id=%u)\n", host_dev_id, src_dev_id, dst_dev_id);
        return DRV_ERROR_PARA_ERROR;
    }
...
static int trs_check_async_devid_dir(
    struct trs_async_dma_input_para *in, uint32_t local_devid, uint32_t src_devid, uint32_t dst_devid, uint32_t dir)
{
    enum trs_async_dma_type async_type = in->async_dma_type;
    int ret = 0;
    ...
    if (async_type == TRS_ASYNC_DMA_TYPE_BATCH) {
        ...
        for (i = 1; i < in->async_batch_in.count; i++) {
            if ((trs_check_batch_async_devid(in->async_batch_in.src[i], src_devid) != DRV_ERROR_NONE) ||
                ...
                trs_err(
                    "Addr invalid. (src_addr=0x%llx; dst_addr=0x%llx; src_devid=%u; dst_devid=%u; index=%u)\n",
...
static int trs_get_check_async_devid_dir(
    uint32_t dev_id, struct trs_async_dma_input_para *in, struct trs_async_devid *async_devid, uint32_t *dir)
{
    enum trs_async_dma_type async_type = in->async_dma_type;
    ...
    if ((srcAttr.devId == host_id) && (dstAttr.devId != host_id)) {
        ...
        *dir = TRS_ASYNC_DEVICE_TO_DEVICE;
        ...
        trs_err(
            "Invalid va. (src=0x%llx; srcMemType=%u; dst=0x%llx; dstMemType=%u; asyncType=%d)\n", src_array[0],
    ...
    trs_debug(
        "(src=0x%llx; src_devid=%u; srcType=%u; dst=0x%llx; dst_devid=%u; dstType=%u; asyncType=%d; devid=%u)\n",
...
static int trs_fill_remote_wr_batch_info(
    struct trs_urma_ctx *urma_ctx, struct trs_async_dma_input_para *in, uint64_t fixedVal,
    struct trs_remote_fill_wqe_info *wqe_fill_info)
{
    ...
    start_index =
        (uint32_t)((async_type == TRS_ASYNC_DMA_TYPE_2D) ? (in->async_2d_in.fixedSize / in->async_2d_in.width) : 0);
...
static inline void trs_get_async_batch_num(
    struct trs_async_dma_input_para *in, uint64_t fixedVal, uint64_t partial_fixed_size, uint32_t *batch_num)
{
    if (in->async_dma_type == TRS_ASYNC_DMA_TYPE_BATCH) {
        ...
        *batch_num =
            (uint32_t)((fixedVal > align_fixed_size) ?
                           ((align_up(fixedVal - align_fixed_size, in->async_2d_in.width) / in->async_2d_in.width)) :
                           0);
...
static int trs_remote_fill_async_dma_wqe(
    struct trs_async_devid *async_devid, struct trs_urma_ctx *urma_ctx, struct trs_async_dma_input_para *in,
    uint64_t fixedVal)
{
...
static int trs_fill_async_dma_direct_wqe(
    struct trs_async_devid *async_devid, struct trs_urma_ctx *urma_ctx, struct trs_async_ctx *async_ctx,
    struct trs_async_dma_wqe_info *wqe_info, struct trs_async_dma_input_para *in)
{
    struct trs_urma_dwqe_fill_para fill_para = {0};
    int ret = 0;

    trs_get_async_send_recv_addr(
        async_devid, (uint64_t)(uintptr_t)in->async_normal_in.src, (uint64_t)(uintptr_t)in->async_normal_in.dst,
        &fill_para.send_addr, &fill_para.recv_addr);
...
static inline uint32_t trs_get_async_jetty_credit(struct trs_async_ctx *async_ctx, bool is_direct_wqe)
{
    if (is_direct_wqe == true) { /* dwqe not post wr, just pack */
        ...
        return trs_jetty_get_credit(
            async_ctx->pi % TRS_STARS_NORMAL_JETTY_DEPTH, async_ctx->ci % TRS_STARS_NORMAL_JETTY_DEPTH,
            TRS_STARS_NORMAL_JETTY_DEPTH);
    }
}

/* fixedVal: means fixedCnt for batch/nop; fixedSIze for others */
static void trs_get_async_post_wr_num(
    struct trs_async_dma_input_para *in, uint32_t credit, bool is_normal_use_fixedVal, uint32_t *wr_num,
    uint64_t *fixedVal, uint64_t *partial_fixed_size)
{
    *wr_num = 0;
    ...
    if ((in->async_dma_type == TRS_ASYNC_DMA_TYPE_NORMAL) || (in->async_dma_type == TRS_ASYNC_DMA_TYPE_SQE_UPDATE)) {
        ...
        *fixedVal = 0;
        for (i = 0; i < in->async_batch_in.count; i++) {
            uint32_t tmp =
                (uint32_t)(align_up(in->async_batch_in.len[i], TRS_URMA_WR_CPY_MAX_SIZE) / TRS_URMA_WR_CPY_MAX_SIZE);
        ...
        u32 align_width_wr_num =
            (uint32_t)(align_up(in->async_2d_in.width, TRS_URMA_WR_CPY_MAX_SIZE) / TRS_URMA_WR_CPY_MAX_SIZE);
        ...
        if (align_fixed_width_wr_num > credit) { /* credit not meet the the-fixedSize-row */
            *wr_num = credit;
            ...
            if ((remain_height - 1) * align_width_wr_num >
                ...
                *wr_num = credit;
                (*fixedVal) +=
                    ((in->async_2d_in.width - width_offset) +
                     (((credit - align_fixed_width_wr_num) / align_width_wr_num) * in->async_2d_in.width) +
                     (((credit - align_fixed_width_wr_num) % align_width_wr_num) * TRS_URMA_WR_CPY_MAX_SIZE));
...
static int trs_fill_async_dma_wqe(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_wqe_info *wqe_info,
    struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out)
{
    ...
    if (credit < wr_num) {
        trs_err(
            "Jetty Credit not enough. (pi=%u; ci=%u; credit=%u; wr_num=%u)\n", async_ctx->pi, async_ctx->ci, credit,
            wr_num);
...
static int trs_fill_h2d_async_dma_wqe(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_wqe_info *wqe_info,
    struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out)
{
    ...
    if (in->async_dma_type == TRS_ASYNC_DMA_TYPE_SQE_UPDATE) {
        ...
        if ((update_sq_info == NULL) || (update_sq_info->urma_ctx == NULL)) {
            trs_err(
                "Invalid para. (devid=%u; tsId=%u; type=%d; sqid=%u)\n", async_devid->local_devid, in->tsId, in->type,
                sqid);
        ...
        if ((sqe_pos >= update_sq_info->depth) ||
            ...
            trs_err(
                "Invalid para. (sqid=%u; sqe_pos=%u; len=%u; depth=%u, sqe_size=%u)\n", sqid, sqe_pos,
        ...
        trs_debug(
            "(devid=%u; sqid=%u; sqe_pos=%u; sqe_size=%u; len=%u; remote_sq_que_addr=0x%llx; dst=0x%llx)\n",
...
static int trs_fill_d2h_async_dma_wqe(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_wqe_info *wqe_info,
    struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out)
{
...
static int trs_alloc_device_jetty(
    uint32_t dev_id, struct trs_d2d_send_info *alloc_info, struct trs_d2d_sync_info *sync_msg)
{
    struct event_reply reply;
    int ret, result;
    ...
    ret = trs_local_mem_event_sync(
        dev_id, alloc_info, sizeof(struct trs_d2d_send_info), DRV_SUBEVENT_TRS_CREATE_ASYNC_JETTY_MSG, &reply);
    ...
    (void)memcpy_s(
        sync_msg, sizeof(struct trs_d2d_sync_info), DRV_EVENT_REPLY_BUFFER_DATA_PTR(reply.buf),
...
static int trs_free_device_jetty(uint32_t dev_id, struct trs_d2d_send_info *free_info)
{
    ...
    reply.buf = (char *)&result;
    ret = trs_local_mem_event_sync(
        dev_id, free_info, sizeof(struct trs_d2d_send_info), DRV_SUBEVENT_TRS_DESTROY_ASYNC_JETTY_MSG, &reply);
    if ((ret != 0) || (result != 0)) {
...
static int trs_import_remote_d2d_jetty(uint32_t dev_id, uint32_t remote_dev_id, struct trs_d2d_dev_ctx *d2d_dev_ctx)
{
    ...
    ret = trs_local_mem_event_sync(
        dev_id, &info, sizeof(struct trs_import_jetty_info), DRV_SUBEVENT_TRS_IMPORT_ASYNC_JETTY_MSG, &reply);
    result = DRV_EVENT_REPLY_BUFFER_RET(reply.buf);
    if ((ret != 0) || (result != 0)) {
        free(reply.buf);
...
static int trs_create_local_d2d_jetty(
    struct trs_async_devid *d2d_async_devid, uint32_t sq_id, uint32_t flag, struct trs_async_ctx *async_ctx)
{
...
static int trs_create_remote_d2d_jetty(uint32_t remote_devid, uint32_t flag, struct trs_d2d_dev_ctx *d2d_dev_ctx)
{
    ...
    trs_debug(
        "Create jetty success. (remote_devid=%u; jfr_id=%u; flag=0x%x)\n", remote_devid, d2d_dev_ctx->dst_jfr_id.id,
        flag);
    return 0;
}
...
static int trs_d2d_dev_ctx_check_init(struct trs_async_devid *d2d_async_devid, uint32_t flag)
{
    ...
    if (d2d_dev_ctx == NULL) {
        ...
        if (ret != 0) {
            trs_err(
                "Failed to create remote jetty. (local_devid=%u; remote_devid=%u; ret=%d; flag=0x%x)\n", local_devid,
                remote_devid, ret, flag);
            free(d2d_dev_ctx);
            goto exit;
        }
        ...
        trs_debug(
            "Create remote jetty success. (remote_devid=%u; flag=0x%x; jetty_id=%u)\n", remote_devid, flag,
...
static int trs_d2d_async_ctx_create(
    struct trs_urma_ctx *urma_ctx, struct trs_async_devid *d2d_async_devid, uint32_t sq_id, uint32_t flag)
{
    uint32_t local_devid = d2d_async_devid->local_devid;
    ...
    if (!async_ctx->init_flag) {
        ...
        create_local_flag = 1;
        trs_debug(
            "Create local jetty success. (devid=%u; flag=0x%x; sq_id=%u; jetty_id=%u; fun_id=%u; die_id=%u)\n",
...
static int trs_fill_d2d_async_dma_wqe(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_wqe_info *wqe_info,
    struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out)
{
...
static int (*const trs_async_dma_wqe_fill_handles[TRS_ASYNC_MAX_DIR])(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_wqe_info *wqe_info,
    struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out) = {
...
static int trs_h2d2h_async_ctx_init(
    uint32_t dev_id, struct trs_urma_ctx *urma_ctx, uint32_t sqId, uint32_t subevent_id,
    struct trs_async_ctx *async_ctx)
{
    ...
    if ((ret != DRV_ERROR_NONE) || (result != 0)) {
        trs_err(
            "Failed to sync event. (dev_id=%u; subevent_id=%u; ret=%d; result=%d)\n", dev_id, subevent_id, ret, result);
        free(reply.buf);
        return (ret != DRV_ERROR_NONE) ? ret : result;
    }

    (void)memcpy_s(
        &jetty_para, sizeof(struct trs_async_jetty_init_para), DRV_EVENT_REPLY_BUFFER_DATA_PTR(reply.buf),
...
static int trs_async_init_async_ctx(
    struct trs_async_devid *async_devid, struct sqcq_usr_info *sq_info, struct trs_async_dma_input_para *in)
{
    struct trs_urma_ctx *urma_ctx = sq_info->urma_ctx;
    int ret = 0;
...
drvError_t trs_async_dma_wqe_create(
    uint32_t dev_id, struct trs_async_dma_input_para *in, struct halAsyncDmaOutputPara *out)
{
    ...
    trs_debug(
        "Create async dma wqe. (dev_id=%u; dir=%u; sq_id=%u; dieid=%u; func_id=%u; jetty_id=%u)\n", dev_id, in->dir,
        in->sqId, out->dieId, out->functionId, out->jettyId);
    return DRV_ERROR_NONE;
}

static drvError_t trs_async_dma_wqe_normal_destory(
    uint32_t dev_id, struct trs_urma_ctx *urma_ctx, struct halAsyncDmaDestoryPara *para)
{
    struct trs_async_dma_wqe_info *wqe_info = NULL;
    struct trs_async_ctx *async_ctx = NULL;
    ...
    trs_debug(
        "Async dma wqe destroy. (dev_id=%u; sq_id=%u; ci=%u; dir=%u)\n", dev_id, para->sqId, async_ctx->ci,
        wqe_info->dir);
    free(wqe_info);
    return 0;
}

static drvError_t trs_async_dma_wqe_batch_2d_destory(
    uint32_t dev_id, struct trs_urma_ctx *urma_ctx, struct trs_async_dma_destroy_para *para)
{
    struct trs_async_ctx *async_ctx = NULL;

    if (para->async_batch_para->ci >= TRS_UB_PI_CI_DEPTH) {
        trs_err(
            "The ci exceed. (dev_id=%u; sq_id=%u; ci=%u; max=%d)\n", dev_id, para->sqId, para->async_batch_para->ci,
            TRS_UB_PI_CI_DEPTH);
        return DRV_ERROR_PARA_ERROR;
    }
...
drvError_t trs_sq_jetty_info_query(uint32_t dev_id, struct halSqCqQueryInfo *info)
{
    ...
    trs_debug(
        "Query jetty info. (devid=%u; sqid=%u; prop=%d; nop_num=%u; jetty_id=%u; func_id=%u; die_id=%u)\n", dev_id,
        info->sqId, info->prop, info->value[0], info->value[1], info->value[2], info->value[3]);

    return DRV_ERROR_NONE;
}

static int trs_async_dma_jetty_remote_alloc(
    uint32_t dev_id, struct halAsyncDmaJettyCreateIn *in, struct trs_async_jetty_info *jetty_info)
{
    ...
    ret = trs_svm_mem_event_sync(
        dev_id, &jetty_para, sizeof(struct trs_remote_alloc_jetty_para), DRV_SUBEVENT_TRS_ALLOC_JETTY_MSG, &reply);
    result = DRV_EVENT_REPLY_BUFFER_RET(reply.buf);
    ...
    (void)memcpy_s(
        jetty_info, sizeof(struct trs_async_jetty_info), DRV_EVENT_REPLY_BUFFER_DATA_PTR(reply.buf),
...
static int trs_async_dma_jetty_remote_free(uint32_t dev_id, struct trs_urma_stars_jetty *slave_jetty_addr)
{
    ...
    ret = trs_local_mem_event_sync(
        dev_id, &slave_jetty_addr, sizeof(struct trs_urma_stars_jetty *), DRV_SUBEVENT_TRS_FREE_JETTY_MSG, &reply);
    if ((ret != DRV_ERROR_NONE) || (result != 0)) {
        trs_err("Failed to sync event. (dev_id=%u; ret=%d; result=%d)\n", dev_id, ret, result);
        return (ret != DRV_ERROR_NONE) ? (int)ret : result;
    }
...
drvError_t trs_async_dma_jetty_create(
    uint32_t dev_id, struct halAsyncDmaJettyCreateIn *in, struct halAsyncDmaJettyCreateOut *out)
{
    ...
    ret = snprintf_s(
        jetty_node->jettyHandle.handle, TRS_ASYNC_DMA_JETTY_HANDLE_LEN, TRS_ASYNC_DMA_JETTY_HANDLE_LEN - 1,
    ...
    ret = hash_table_put2(
        g_jetty_mng_ctx[dev_id].hash_table, jetty_node->jettyHandle.handle, jetty_node, trs_async_jetty_node_free);
    if (ret != 0) {
...
drvError_t trs_async_dma_jetty_query(
...
static int trs_async_dma_nop_wqe_convert(
    struct trs_async_dma_input_para *input, uint8_t *wqe_buffer, uint64_t wqe_bufferLen,
...
static int _trs_async_dma_copy_wqe_convert(
    struct trs_async_devid *async_devid, uint32_t dir, struct trs_async_addr_pack *addr_pack, uint8_t *wqe_buffer,
    bool is_last_batch)
{
    struct trs_urma_dwqe_fill_para fill_para = {0};
    ...
    if (ret != 0) {
        trs_err(
            "Get token_id from svm failed. (devid=%u; va=0x%llx; len=0x%llx; ret=%d)\n", devid, recv_addr,
            addr_pack->len, ret);
        return ret;
    }
    ...
    for (i = 0; i < wqe_num; i++) {
        ...
        fill_para.len =
            (i == (wqe_num - 1)) ? (addr_pack->len - i * TRS_URMA_WR_CPY_MAX_SIZE) : TRS_URMA_WR_CPY_MAX_SIZE;
...
static int trs_async_dma_copy_wqe_convert(
    struct trs_async_devid *async_devid, struct trs_async_dma_input_para *input, uint8_t *wqe_buffer,
    uint64_t wqe_bufferLen, struct halAsyncDmaWqeOutputPara *out)
{
    ...
    for (i = 0; i < batch_num; i++) {
        ...
        ret =
            _trs_async_dma_copy_wqe_convert(async_devid, input->dir, &addr_pack, dst_wqe_addr, (i == (batch_num - 1)));
        if (ret != 0) {
            return ret;
        }

        dst_wqe_addr +=
            (align_up(addr_pack.len, TRS_URMA_WR_CPY_MAX_SIZE) / TRS_URMA_WR_CPY_MAX_SIZE) * STARS_ASYNC_DMA_WQE_SIZE;
    }
...
drvError_t trs_async_dma_wqe_convert(
...
drvError_t trs_async_dma_jetty_fill(uint32_t dev_id, struct halAsyncDmaJettyFillInfo *para)
{
    ...
    if ((para->offset + para->size) > jetty_node->jetty_info.jetty_queue_len) {
        (void)pthread_rwlock_unlock(&g_jetty_mng_ctx[dev_id].rw_mutex);
        trs_err(
            "Failed to fill jetty queue. (dev_id=%u; offset=0x%llx; size=0x%llx; len=0x%x; ret=%d)\n", dev_id,
            para->offset, para->size, jetty_node->jetty_info.jetty_queue_len, ret);
        return DRV_ERROR_PARA_ERROR;
    }

    ret = halSvmAccess(
        dev_id, jetty_node->jetty_info.jetty_queue_base + para->offset, (uint64_t)(uintptr_t)para->srcWqe, para->size,
        SVM_MEM_ACCESS_WRITE);
    if (ret != DRV_ERROR_NONE) {
        (void)pthread_rwlock_unlock(&g_jetty_mng_ctx[dev_id].rw_mutex);
        trs_err(
            "Failed to fill jetty queue. (dev_id=%u; addr=0x%llx; offset=0x%llx; src=%px; len=0x%llx; ret=%d)\n",
