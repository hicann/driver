...
{
    if (svm_dbi_is_support_pci_bar(devid)) {
...
static bool mem_ctrl_is_pcie_bar_huge_mem_support(u32 devid)
{
    return svm_dbi_is_support_pci_bar(devid);
}
    ...
    void *param_value, size_t param_value_size, void *out_value, size_t *out_size_ret)
{
    ...
    tmp_module_id = (svm_flag_get_module_id(prop.flag) == SVM_FLAG_INVALID_MODULE_ID) ?
                        SVM_INVALID_MODULE_ID :
                        svm_flag_get_module_id(prop.flag);
