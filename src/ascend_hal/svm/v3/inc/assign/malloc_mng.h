...
struct svm_priv_ops {
    ...
    bool (*should_criu_reset)(void *priv); /* not must, release the inited handle during criu reset when return true */
};
 ...
 * 4. Will call priv->release(force=false), if priv->release return DRV_ERROR_BUSY, svm_free will return
